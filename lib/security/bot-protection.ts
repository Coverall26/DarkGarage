...
> {
  try {
    ...
    logger.error("BotID verification error", { module: "bot-protection", error: String(error) });
    return { blocked: false };
  }
}
...
export async function verifyNotBotPages(
  ...
  try {
    ...
    logger.error("BotID verification error (pages)", { module: "bot-protection", error: String(error) });
    return true;
  }
}
