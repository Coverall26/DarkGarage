...
/** iOS Safari navigator.standalone — non-standard property */
...
export function PWAInstallPrompt() {
  ...
  useEffect(() => {
    ...
    if (isDevEnvironment) {
      ...
      }).catch((e) => logger.error("SW error", { module: "pwa-install", metadata: { error: (e as Error).message } }));
      return;
    }
    ...
    navigator.serviceWorker.register('/sw.js', { updateViaCache: 'none' }).then((registration) => {
      updateInterval = setInterval(() => {
        registration.update().catch((e) => logger.error("SW error", { module: "pwa-install", metadata: { error: (e as Error).message } }));
      }, 60 * 60 * 1000);
