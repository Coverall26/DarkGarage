...
**Step 7: Integrations + Compliance** — Active defaults (SignSuite, S3, audit, Resend, manual wire), optional integrations (KYC, ACH, accounting, tax — Coming Soon), compliance settings (audit retention 7yr default, export format, Form D reminder)
...
**Step 2: NDA E-Signature** — Display NDA, SignSuite, or upload external (pending GP confirmation)
...
**Step 5: Commitment + Signing** — Investment amount, 12 required subscription representations (each audit-logged), e-sign via SignSuite OR upload external docs (pending GP confirmation). Entity authorized signer + custodian co-sign for IRA.
...
- Step 2: NDA e-signature (SignSuite)
