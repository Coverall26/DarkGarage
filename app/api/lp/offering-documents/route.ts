...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (investor.fund) {
      const fundFeatureFlags = investor.fund.featureFlags as Record<string, unknown> | null;
      if (
        fundFeatureFlags?.offeringDocuments &&
        Array.isArray(fundFeatureFlags.offeringDocuments)
      ) {
        documents = fundFeatureFlags.offeringDocuments as OfferingDocument[];
      }
    }
