...
export async function POST(
  ...
  try {
    ...
    if (action === "verify") {
      return handleVerify(req, link as WorkflowLink);
    }

    // Handle access action
    if (action === "access") {
      return handleAccess(req, link as WorkflowLink);
    }
...
async function handleVerify(req: NextRequest, link: WorkflowLink) {
...
async function handleAccess(req: NextRequest, link: WorkflowLink) {
  const body = await req.json();
