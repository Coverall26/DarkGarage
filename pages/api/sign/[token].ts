...
async function handleGet(
  ...
  try {
    ...
    try {
      ...
      logger.error("Error getting file URL", { module: "sign", metadata: { error: (error as Error).message } });
    }
    ...
    logger.error("Error fetching signing document", { module: "sign", metadata: { error: (error as Error).message } });
...
async function handlePost(
  ...
  try {
    ...
    if (!anomalyAllowed) {
      logger.warn(`[SECURITY] Blocked signing attempt for recipient ${recipient.id} due to anomalies`, { module: "sign", metadata: { recipientId: recipient.id, alerts: String(alerts) } });
    ...
    try {
      ...
      reportError(error as Error);
      logger.error("Failed to fetch document for checksum", { module: "sign", metadata: { error: (error as Error).message } });
    ...
    if (signatureImage) {
      try {
        ...
        logger.error("Failed to encrypt signature, storing unencrypted", { module: "sign", metadata: { error: (encryptError as Error).message } });
    ...
    if (result.allSigned && result.newStatus === "COMPLETED") {
      ...
      // Fire-and-forget: Record e-sig usage for tier enforcement
      import("@/lib/esig/usage-service").then(({ recordDocumentCompleted }) =>
        recordDocumentCompleted(document.teamId)
      ).catch((e) => reportError(e as Error));
        ...
        flattenSignatureDocument({ documentId: document.id, saveToStorage: true })
          .then((flattenResult) => {
            if (!flattenResult.success) {
              ...
              logger.error(`[SIGN] PDF flatten failed for document ${document.id}:`, { module: "sign", metadata: { error: String(flattenResult.error) } });
            }
          })
          .catch((err) => {
            reportError(err as Error);
            logger.error("[SIGN] PDF flatten error", { module: "sign", metadata: { error: (err as Error).message } });
          });
      }).catch((err) => {
        reportError(err as Error);
        logger.error("[SIGN] PDF flatten import error", { module: "sign", metadata: { error: (err as Error).message } });
      });
      ...
      processDocumentCompletion(document.id, {
        ...
        if (!encryptionResult.success) {
          ...
          logger.error(`[SECURITY] Document encryption failed for document ${document.id}`, { module: "sign" });
        }
      }).catch((err) => {
        reportError(err as Error);
        logger.error("[SECURITY] Document encryption error", { module: "sign", metadata: { error: (err as Error).message } });
      });
      ...
      for (const r of result.allRecipients) {
        emailPromises.push(
          sendOrgEmail({
            ...
            reportError(err as Error);
            logger.error(`Failed to send completion email to ${r.email}:`, { module: "sign", metadata: { error: (err as Error).message } });
          })
        );
      }
      ...
      if (document.owner?.email) {
        emailPromises.push(
          sendOrgEmail({
            ...
            reportError(err as Error);
            logger.error(`Failed to send completion email to owner:`, { module: "sign", metadata: { error: (err as Error).message } });
          })
        );
      }
          ...
          import("@/lib/investors/advance-on-signing-complete").then(
            ({ advanceInvestorOnSigningComplete }) => {
              advanceInvestorOnSigningComplete({
                ...
                reportError(err as Error);
                logger.error("[SIGN] Investor advancement check failed", { module: "sign", metadata: { error: (err as Error).message } });
              });
            }
          ).catch((err) => {
            reportError(err as Error);
            logger.error("[SIGN] Investor advancement import failed", { module: "sign", metadata: { error: (err as Error).message } });
          });
        }
      } catch (lpError) {
        reportError(lpError as Error);
        logger.error("Failed to store document in LP vault", { module: "sign", metadata: { error: (lpError as Error).message } });
      }
      ...
      if (document.documentType === "SUBSCRIPTION" && document.subscriptionAmount) {
        try {
          ...
          logger.error("Failed to process subscription completion", { module: "sign", metadata: { error: (subError as Error).message } });
        }
      }
    ...
    logger.error("Error processing signature", { module: "sign", metadata: { error: (error as Error).message } });
    return res.status(500).json({ error: "Internal server error" });
  }
}
