...
async function createDataroomSession(
  ...
  if (!redis) {
    ...
    logger.warn("Redis not configured - dataroom sessions will not persist across page loads", { module: "dataroom-auth" });
    return null;
  }
...
async function verifyDataroomSession(
  ...
  try {
    ...
    logger.error("Redis error", { module: "dataroom-auth", error: String(error) });
  ...
  try {
    ...
    logger.error("Session validation error", { module: "dataroom-auth", error: String(error) });
