...
export default function GPDashboardClient() {
  ...
  useEffect(() => {
    ...
    fetch("/api/admin/team-context", { signal: controller.signal })
      ...
      .catch((e) => { if (e?.name !== "AbortError") logger.error("Failed to load org mode", { module: "admin-dashboard", metadata: { error: (e as Error).message } }); });
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
        ...
        <Button
          ...
          style={{ backgroundColor: SUITE_COLORS.fundroom }}
    ...
    <div className="animate-fade-in space-y-5">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        ...
        <Button
          ...
          className="min-h-[44px] sm:min-h-0 sm:h-8 text-xs"
        >
        ...
        <div className="flex items-center gap-2 overflow-x-auto scrollbar-hide pb-0.5">
          {dataroomLink ? (
            <button
              ...
              className="inline-flex items-center gap-1.5 min-h-[44px] sm:min-h-0 sm:h-8 px-3 rounded-md text-xs font-medium border transition-colors whitespace-nowrap"
              style={{ borderColor: `${SUITE_COLORS.dataroom}4D`, backgroundColor: `${SUITE_COLORS.dataroom}0D`, color: SUITE_COLORS.dataroom }}
            >
              {linkCopied ? (
          ...
          <Link href="/admin/investors/new">
            <span className="inline-flex items-center gap-1.5 min-h-[44px] sm:min-h-0 sm:h-8 px-3 rounded-md text-xs font-medium border border-border bg-background hover:bg-muted transition-colors whitespace-nowrap">
      ...
      {isFirstTime && (
        <Card className="shadow-sm" style={{ borderColor: `${SUITE_COLORS.fundroom}33`, backgroundColor: `${SUITE_COLORS.fundroom}0D` }}>
          <CardContent className="py-8 text-center">
            <div className="h-14 w-14 rounded-2xl flex items-center justify-center mx-auto mb-4" style={{ backgroundColor: `${SUITE_COLORS.fundroom}1A` }}>
              <modeConfig.icon className="h-7 w-7" style={{ color: SUITE_COLORS.fundroom }} />
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-2 justify-center">
              <Link href={modeConfig.emptyFundHref}>
                <Button className="hover:opacity-90 text-white w-full sm:w-auto" style={{ backgroundColor: SUITE_COLORS.fundroom }}>
