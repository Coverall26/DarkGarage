...
jest.mock("@/lib/id-helper", () => ({
  newId: jest.fn().mockReturnValue("00000000-0000-4000-a000-000000000070"),
}));
...
describe("Production Smoke Tests (P3-6)", () => {
  ...
  describe("1. Health & Deployment Readiness", () => {
    ...
    it("unauthenticated requests are rejected on admin endpoints", async () => {
      setSession(null);
      // Override the RBAC mock to simulate unauthenticated access
      const { requireAdminAppRouter } = jest.requireMock("@/lib/auth/rbac");
      requireAdminAppRouter.mockResolvedValueOnce(
  ...
  describe("6. Auth & Security Guards", () => {
    ...
    it("LP endpoints require authentication", async () => {
      setSession(null);
      // Override the RBAC mock so requireLPAuthAppRouter returns 401
      const rbac = jest.requireMock("@/lib/auth/rbac");
      rbac.requireLPAuthAppRouter.mockResolvedValueOnce(
        NextResponse.json({ error: "Unauthorized" }, { status: 401 }),
      );
    ...
    it("rate limiting blocks excessive requests", async () => {
      // Override the rate limiter mock to return a 429 response
      const rateLimiter = jest.requireMock("@/lib/security/rate-limiter");
      rateLimiter.appRouterStrictRateLimit.mockResolvedValueOnce(
        NextResponse.json(
  ...
  describe("7. Dashboard Stats & Pending Actions", () => {
    it("GP dashboard stats returns fund metrics", async () => {
      // dashboard-stats uses requireAdminAppRouter (mocked globally) then userTeam.findMany
      setSession(GP_SESSION);
