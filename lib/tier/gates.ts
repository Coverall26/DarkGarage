...
// ---------------------------------------------------------------------------
// Signature template limit
// ---------------------------------------------------------------------------

/**
 * Check if org can create more signature templates.
 * FREE: 2, CRM_PRO: 5, FUNDROOM: unlimited
 */
export async function checkSignatureTemplateLimit(orgId: string): Promise<GateResult> {
  const tier = await resolveOrgTier(orgId);
  if (tier.signatureTemplateLimit === null) {
    return { allowed: true };
  }

  // Count templates across all teams in this org
  const teams = await prisma.team.findMany({
    where: { organizationId: orgId },
    select: { id: true },
  });
  ...
  const used = teamIds.length > 0
    ? await prisma.signatureTemplate.count({ where: { teamId: { in: teamIds } } })
    : 0;

  if (used >= tier.signatureTemplateLimit) {
    return {
      allowed: false,
      error: "SIGNATURE_TEMPLATE_LIMIT_REACHED",
      meta: {
        used,
        limit: tier.signatureTemplateLimit,
        upgradeUrl: "/admin/settings?tab=billing",
      },
    };
  }

  return { allowed: true, meta: { used, limit: tier.signatureTemplateLimit } };
}

/**
 * Get signature template usage for display in the UI.
 */
export async function getSignatureTemplateUsage(orgId: string): Promise<{
  limit: number | null;
  used: number;
  canCreate: boolean;
}> {
  ...
  const limit = tier.signatureTemplateLimit;
