...
export default async function handle(
  ...
  if (req.method === "POST") {
    ...
    try {
      ...
      const results = await Promise.all(
        normalizedEmails.map(async (email) => {
          try {
            ...
            logger.error(`Failed to send invite to ${email}:`, { module: "teams", metadata: { error: (error as Error).message } });
      ...
      logger.error("Quick Add invite error", { module: "teams", metadata: { error: (error as Error).message } });
