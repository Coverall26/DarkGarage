...
async function logAnomalies(alerts: AnomalyAlert[]) {
  try {
    ...
    logger.error("Failed to log anomaly alerts", { module: "security", error: String(error) });
  }
}
...
async function sendSecurityAlert(alerts: AnomalyAlert[]) {
  ...
  logger.error("Security alert", { module: "security", alertMessage: message });
  ...
  try {
    ...
    logger.error("Failed to log security alert", { module: "security", error: String(error) });
  }
}
