  ...
  try {
    ...
    for (const adminEmail of adminEmails) {
      ...
      if (!quickAddMagicLink) {
        logger.error("[REQUEST_INVITE] Failed to create quick-add magic link", { module: "request-invite", metadata: { adminEmail } });
      }
      if (!dashboardMagicLink) {
        logger.error("[REQUEST_INVITE] Failed to create dashboard magic link", { module: "request-invite", metadata: { adminEmail } });
      }
    ...
    logger.error("Error sending invite request", { module: "request-invite", metadata: { error: (error as Error).message } });
