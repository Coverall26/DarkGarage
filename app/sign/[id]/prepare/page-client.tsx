...
export default function PrepareDocument() {
  ...
  useEffect(() => {
    ...
    if (document?.fields) {
      setFields(
        document.fields.map((f: { id: string; type: string; recipientId?: string | null; pageNumber: number; x: number; y: number; width: number; height: number; label?: string | null; required?: boolean }) => ({
          id: f.id,
          type: f.type,
  ...
  const updateFieldProperty = (fieldId: string, property: keyof PlacedField, value: PlacedField[keyof PlacedField]) => {
  ...
  const getRecipientColor = (recipientId: string) => {
    ...
    const index =
      document?.recipients?.findIndex((r: { id: string }) => r.id === recipientId) || 0;
            ...
            <Card>
              ...
              <CardContent>
                ...
                >
                  ...
                  <SelectContent>
                    {document.recipients?.map((recipient: { id: string; name: string }) => (
                  ...
                  <div className="space-y-2">
                    {fields.map((field) => {
                      const recipient = document.recipients?.find(
                        (r: { id: string }) => r.id === field.recipientId
                      );
                      return (
                        <div
                          key={field.id}
