...
export const sendVerificationRequestEmail = async (params: {
  ...
  if (!isResendConfigured()) {
    logger.error("Cannot send verification email — RESEND_API_KEY is not set", { module: "send-verification-request", email: email.substring(0, 3) + "***" });
  ...
  try {
    ...
    logger.error("Error sending verification email", { module: "send-verification-request", error: String(e), email: email.substring(0, 3) + "***" });
