...
import type { Action, Condition, ConditionsConfig } from "@/ee/features/workflows/lib/types";
...
export async function PATCH(
  ...
  try {
    ...
    let validatedConditions: ConditionsConfig | undefined;
    ...
    let validatedActions: Action[] | undefined;
    ...
    if (validatedConditions) {
      ...
      if (validatedConditions.items) {
        validatedConditions.items.forEach((condition: Condition) => {
    ...
    if (validatedActions) {
      ...
      const existingActions = existingStep?.actions as Action[] | undefined;
    ...
    const updates: import("@prisma/client").PrismaPromise<unknown>[] = [
      prisma.workflowStep.update({
        where: { id: stepId },
        data: updateData as Parameters<typeof prisma.workflowStep.update>[0]["data"],
      }),
    ];
