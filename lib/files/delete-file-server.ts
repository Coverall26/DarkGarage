...
export const deleteFile = async ({ type, data, teamId }: DeleteFileOptions) => {
  ...
  return await match(type)
    .with(DocumentStorageType.S3_PATH, async () => {
      ...
      logger.warn("Skipping S3 deletion (not configured for S3)", { module: "delete-file-server", data });
      return;
    })
...
const deleteFileFromVercelServer = async (url: string) => {
  ...
  logger.warn("Skipping Vercel Blob deletion (not configured)", { module: "delete-file-server", url });
  return;
};
...
const deleteAllFilesFromS3Server = async (data: string, teamId: string) => {
  ...
  try {
    ...
    logger.error("Error deleting files from S3", { module: "delete-file-server", teamId, error: String(error) });
  }
};
