...
export async function sendInvestorApprovedEmail(
  ...
  try {
    ...
    if (!investor?.user?.email) {
      logger.warn("No email found for approved notification", { module: "send-investor-approved", investorId });
      return;
    }
    ...
    logger.error("Failed to send approved notification", { module: "send-investor-approved", error: String(error), investorId, fundId });
  }
}
...
export async function sendWireInstructionsIfAvailable(
  ...
  try {
    ...
    logger.error("Failed to send wire instructions email", { module: "send-investor-approved", error: String(error), email, fundId });
  }
}
