...
import { StartupRaiseCreateSchema } from "@/lib/validations/esign-outreach";
...
export async function POST(req: NextRequest) {
  ...
  try {
    const wrapperParsed = await validateBody(req, StartupRaiseCreateSchema);
    if (wrapperParsed.error) return wrapperParsed.error;
    const { teamId, ...raiseData } = wrapperParsed.data;
