...
export async function setUserIdForCache(userId: string): Promise<void> {
  try {
    ...
    logger.error("Failed to set user ID for cache", { module: "offline-cache", metadata: { error: (error as Error).message } });
  }
}
...
export async function cacheDocumentForOffline(
  ...
  try {
    ...
    logger.error("Failed to cache document", { module: "offline-cache", metadata: { error: (error as Error).message } });
    return { success: false, error: (error as Error).message };
  }
}
...
export async function removeCachedDocument(
  ...
  try {
    ...
    logger.error("Failed to remove cached document", { module: "offline-cache", metadata: { error: (error as Error).message } });
    return { success: false, error: (error as Error).message };
  }
}
...
export async function getCachedDocuments(userId?: string): Promise<CachedDocument[]> {
  try {
    ...
    logger.error("Failed to get cached documents", { module: "offline-cache", metadata: { error: (error as Error).message } });
    return [];
  }
}
...
export async function getCacheStats(userId?: string): Promise<CacheStats> {
  try {
    ...
    logger.error("Failed to get cache stats", { module: "offline-cache", metadata: { error: (error as Error).message } });
...
export async function isDocumentCached(id: string, userId?: string): Promise<boolean> {
  try {
    ...
    logger.error("Failed to check document cache status", { module: "offline-cache", metadata: { error: (error as Error).message } });
    return false;
  }
}
...
export async function clearDocumentCache(userId?: string): Promise<{ success: boolean; error?: string }> {
  try {
    ...
    logger.error("Failed to clear document cache", { module: "offline-cache", metadata: { error: (error as Error).message } });
    return { success: false, error: (error as Error).message };
  }
}
