...
export async function GET(req: NextRequest) {
  try {
    ...
    type FundProgress = NonNullable<Awaited<ReturnType<typeof getFundProgress>>>;
    const trancheProgressMap = new Map<string, FundProgress>();
    await Promise.all(
    ...
    const fundData = funds.map((fund) => {
      ...
      const manualCommitments = fundManualInvestments.reduce(
        (sum, mi) => sum + Number(mi.commitmentAmount),
        0
      );
      ...
      const manualFunded = fundManualInvestments.reduce(
        (sum, mi) => sum + Number(mi.fundedAmount),
        0
      );
      ...
      fundManualInvestments.forEach((mi) => totalInvestors.add(mi.investorId));

      const fundInvestorIds = new Set<string>();
      fund.investments.forEach((inv) => fundInvestorIds.add(inv.investorId));
      fundManualInvestments.forEach((mi) => fundInvestorIds.add(mi.investorId));
