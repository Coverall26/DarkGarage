...
export function createSafeValidator(
  ...
  for (const pattern of dangerousPatterns) {
    if (pattern.test(validatorCode)) {
      logger.warn("Dangerous pattern detected in validator code", { module: "sandbox", pattern: String(pattern) });
      return null;
    }
  }

  if (validatorCode.length > 2000) {
    logger.warn("Validator code too long", { module: "sandbox", length: validatorCode.length });
    return null;
  }
  ...
  return (value: string, context: Record<string, unknown>): boolean | string => {
    try {
      ...
      logger.error("Validator execution error", { module: "sandbox", error: String(error) });
