...
import { checkModuleAccess } from "@/lib/middleware/module-access";
...
export async function GET(
  ...
  try {
    ...
    // Module access check — RAISE_CRM required for engagement analytics
    if (ctx.team.organizationId) {
      const moduleBlocked = await checkModuleAccess(ctx.team.organizationId, "RAISE_CRM");
...
export async function POST(
  ...
  try {
    ...
    // Module access check — RAISE_CRM required for engagement recalculation
    if (ctx.team.organizationId) {
      const moduleBlocked = await checkModuleAccess(ctx.team.organizationId, "RAISE_CRM");
      if (moduleBlocked) return moduleBlocked;
    }
