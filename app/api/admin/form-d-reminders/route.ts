...
import { FormDReminderActionSchema } from "@/lib/validations/admin";
...
export async function POST(req: NextRequest) {
  ...
  const parsed = await validateBody(req, FormDReminderActionSchema);
  if (parsed.error) return parsed.error;
  const { fundId, action, formDFilingDate, formDAmendmentDue } = parsed.data;
