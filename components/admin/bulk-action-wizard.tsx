  ...
  const fetchInvestors = async () => {
    try {
      ...
      if (res.ok) {
        ...
        const investorData = (data.investors || []).map((inv: Record<string, unknown>) => ({
          ...inv,
          hasBankLink: true,
        }));
      ...
      logger.error("Failed to fetch investors", { module: "bulk-action-wizard", metadata: { error: (error as Error).message } });
    } finally {
      setLoading(false);
    }
  };
