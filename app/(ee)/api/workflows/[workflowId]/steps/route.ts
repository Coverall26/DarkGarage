...
import type { Action, Condition, ReorderStepsRequest } from "@/ee/features/workflows/lib/types";
  ...
  try {
    ...
    const enrichedSteps = await Promise.all(
      steps.map(async (step) => {
        const actions = step.actions as Action[];
...
export async function POST(
  ...
  try {
    ...
    if (conditionsValidation.data.items) {
      conditionsValidation.data.items.forEach((condition: Condition) => {
    ...
    const transactionSteps: import("@prisma/client").PrismaPromise<unknown>[] = [
      prisma.workflowStep.create({
        data: {
          ...
          stepType: "ROUTER",
          conditions: conditionsValidation.data as unknown as Parameters<typeof prisma.workflowStep.create>[0]["data"]["conditions"],
          actions: actionsValidation.data as unknown as Parameters<typeof prisma.workflowStep.create>[0]["data"]["actions"],
        },
      }),
    ];
