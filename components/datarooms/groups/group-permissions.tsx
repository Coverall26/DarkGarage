...
import { CloudDownloadOff } from "@/components/shared/icons";
...
const buildTree = (
  items: DataroomFolderWithDocuments[],
  ...
  // Note: items are DataroomFolderWithDocuments (folders). They have a `.documents` array, not `.document` singular.
  items
    ...
    .forEach((folder) => {
      ...
      const folderDocuments = (folder.documents || []).map((doc) => ({
        id: doc.id,
  ...
  // Handle root-level documents (documents in folders at the current parentId level)
  // Documents are nested inside folder.documents, not as top-level items
  if (parentId === null) {
    // For root level, find documents that belong to root-level folders' direct documents
    // (folders at root have parentId === null and their documents are root-level)
    items
      .filter((item) => item.parentId === null)
      .forEach((folder) => {
        // Documents in this folder are already handled above via folderDocuments
        // This section handles orphaned documents if any exist outside folder structure
      });
  }
