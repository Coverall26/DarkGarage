  ...
  private async trackEvent(eventType: EventType, endTime?: number, useBeacon: boolean = false) {
    ...
    if (useBeacon && navigator.sendBeacon) {
      try {
        ...
        logger.warn("sendBeacon failed", { module: "video-tracking", metadata: { error: (error as Error).message } });
      }
    }
    ...
    try {
      ...
      logger.warn("Fetch with keepalive failed", { module: "video-tracking", metadata: { error: (error as Error).message } });
    }
    ...
    if (!useBeacon && navigator.sendBeacon) {
      try {
        ...
        logger.warn("Fallback sendBeacon failed", { module: "video-tracking", metadata: { error: (error as Error).message } });
      }
    }
