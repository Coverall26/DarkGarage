...
/** Shape of the Form D export data object, matching SEC Form D (OMB 3235-0076) sections. */
interface FormDData {
  issuer: {
    entityName: string; entityType: string; ein: string;
    yearOfIncorporation: number | null; stateOfIncorporation: string;
  ...
  relatedPersons: Array<{
    ...
    title?: string; address?: string; relationship?: string;
  }>;
  industryGroup: string;
  issuerSize: { revenueRange: string; aggregateNetAssetValue: string };
  federalExemption: {
    regulationDExemption: string; exemptionLabel: string;
    rule506b: boolean; rule506c: boolean;
    regulationAPlus: boolean; rule504: boolean;
  };
  filingType: string;
  previousFilingDate: string | null;
  offeringDuration: { indefinite: boolean; longerThanOneYear: boolean };
  securitiesType: string;
  businessCombination: boolean;
  minimumInvestment: number;
  salesCompensation: unknown[];
  offeringAmounts: {
    totalOfferingAmount: number; totalAmountSold: number;
    totalAmountRemaining: number; clarificationOfResponse: string | null;
  };
  investorCounts: {
    totalAlreadyInvested: number; totalAccredited: number;
    totalNonAccredited: number;
    accreditationBreakdown: {
      selfCertified: number; kycVerified: number;
      thirdPartyVerified: number; pending: number; nonAccredited: number;
    };
  };
  salesCommissions: {
    totalSalesCommissions: string | number;
    totalFindersFees: number; clarification: string | null;
  };
  useOfProceeds: { generalDescription: string; estimated: boolean };
  meta: {
    fundName: string; fundId: string; fundStatus: string;
    fundMode: string; fundSubType: string | null;
    firstSaleDate: string | null; filingDeadline: string | null;
    existingFilingDate: string | null; amendmentDue: string | null;
    exportDate: string; exportedBy: string;
    managementFeePct: number | null; carryPct: number | null;
    hurdleRate: number | null; termYears: number | null;
  };
}
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    return errorResponse(error, "admin-reports-form-d");
  }
}
...
function buildFormDCsv(data: FormDData): string {
  const lines: string[] = [];
  const d = data;
