    ...
    (resolve, reject) => {
      ...
      const upload = new tus.Upload(file, {
        ...
        onError: (error) => {
          ...
          logger.error("Viewer TUS upload failed", { module: "viewer-tus-upload", error: String(error) });
          reject(error);
        },
        onShouldRetry(error, retryAttempt, options) {
          logger.warn("Retrying viewer TUS upload", { module: "viewer-tus-upload", retryAttempt });
      ...
      upload
        ...
        .catch((error) => {
          logger.error("Error finding previous viewer uploads", { module: "viewer-tus-upload", error: String(error) });
          upload.start();
          resolve({ upload, complete });
        });
    },
