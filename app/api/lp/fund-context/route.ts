 ...
 *
 * INTENTIONALLY_PUBLIC: Returns limited fund/org context for LP onboarding.
 ...
 * BEFORE the LP has an active session. Adding LP auth here would break
 * the onboarding flow. Rate limiting + paywall check provide protection.
 */
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Check if KYC is required for this team's activation
    let kycRequired = false;
    try {
      const activation = await prisma.fundroomActivation.findFirst({
        where: { teamId: team.id, status: "ACTIVE" },
        select: { kycRequired: true },
      });
      kycRequired = activation?.kycRequired ?? false;
    } catch (error) {
      // If FundroomActivation model doesn't have kycRequired yet, default false
    }
    ...
    return errorResponse(error, "lp-fund-context");
  }
}
