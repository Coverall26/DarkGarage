...
import { DraftEmailSchema } from "@/lib/validations/esign-outreach";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const parsed = await validateBody(req, DraftEmailSchema);
    if (parsed.error) return parsed.error;
    const { contactId, purpose, additionalContext } = parsed.data;
