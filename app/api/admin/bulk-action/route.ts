...
import { BulkActionSchema } from "@/lib/validations/admin";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const parsed = await validateBody(req, BulkActionSchema);
    if (parsed.error) return parsed.error;
    const { fundId, actionType, totalAmount, allocationType, selectedInvestors } = parsed.data;
    ...
    const allocations = investors.map(
      (inv: {
        ...
        if (allocationType === "equal") {
          allocationAmount = totalAmount / investors.length;
        } else {
          const share = Number(inv.commitmentAmount) / totalCommitments;
          allocationAmount = totalAmount * share;
        }
