...
export async function checkAndMarkThresholdReached(
  fundId: string
): Promise<{ reached: boolean; shouldNotify: boolean; fund: Awaited<ReturnType<typeof prisma.fund.findUnique>> }> {
  const fund = await prisma.fund.findUnique({
    where: { id: fundId },
    include: { aggregate: true },
  });
  ...
  const alreadyNotified = fund.aggregate?.initialThresholdMet || 
    (fund.customSettings as Record<string, unknown> | null)?.thresholdNotifiedAt;
...
export async function markThresholdNotified(fundId: string): Promise<void> {
  ...
  if (fund?.aggregate) {
    ...
    const currentSettings = (fund.customSettings as Record<string, unknown>) || {};
    await prisma.fund.update({
      where: { id: fundId },
      data: {
        customSettings: {
