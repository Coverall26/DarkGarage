...
const CustomTooltip = ({
  ...
  payload: Array<{ value: number; dataKey: string; payload: { pageNumber: string; versionNumber: string } }>;
  ...
  return (
    <>
      <div className="w-52 rounded-md border border-tremor-border bg-tremor-background text-sm leading-6 dark:border-dark-tremor-border dark:bg-dark-tremor-background">
        ...
        {payload.map((item: { value: number; dataKey: string; payload: { pageNumber: string; versionNumber: string } }, idx: number) => (
          <div
