...
import { apiRateLimiter, strictRateLimiter } from "@/lib/security/rate-limiter";
...
async function handleGet(req: NextApiRequest, res: NextApiResponse, user: AuthenticatedUser) {
  try {
    ...
    logger.error("Error fetching transactions", { module: "transactions", metadata: { error: (error as Error).message } });
...
async function handlePost(req: NextApiRequest, res: NextApiResponse, user: AuthenticatedUser) {
  try {
    ...
    reportError(error as Error);
    logger.error("Error creating transaction", { module: "transactions", metadata: { error: (error as Error).message } });
    return res.status(500).json({ error: "Internal server error" });
  }
}
