/**
 * ACTIVE — CRM Billing Settings Section
 *
 * Production billing UI for the 4-tier CRM subscription system (FREE → CRM_PRO → BUSINESS → FUNDROOM).
 * Renders plan comparison cards, AI CRM add-on management, usage meters, and Stripe billing portal link.
 *
 * Data sources:
 *   GET /api/tier           — current tier + AI trial info
 *   GET /api/billing/usage  — contacts/e-sigs/templates usage with limits
 *   POST /api/billing/checkout  — Stripe Checkout session for upgrades
 *   POST /api/billing/ai-addon  — subscribe/cancel AI CRM add-on
 *   POST /api/billing/portal    — Stripe Billing Portal redirect
 *
 * This is the CRM billing system (org-scoped, at lib/stripe/crm-products.ts),
 * separate from the SaaS billing system (team-scoped, at ee/stripe/).
 *
 * Phase 2: Unify SaaS + CRM billing into a single subscription model.
 * Phase 2: Add UpgradePlanModal integration (components/billing/upgrade-plan-modal.tsx).
 */
"use client";
