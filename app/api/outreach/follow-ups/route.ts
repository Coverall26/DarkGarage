...
import { FollowUpScheduleSchema } from "@/lib/validations/esign-outreach";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const teamId = userTeam.team.id;
    const parsed = await validateBody(req, FollowUpScheduleSchema);
    if (parsed.error) return parsed.error;
    const { contactId, followUpAt, notes } = parsed.data;
