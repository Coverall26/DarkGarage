...
export function PoweredByFooter({ theme = "dark", className = "" }: { theme?: "dark" | "light"; className?: string }) {
  return (
    <div className={`flex items-center justify-center py-3 ${className}`}>
      ...
      >
        <span className={`text-xs font-medium tracking-wide uppercase ${theme === "dark" ? "text-gray-400" : "text-gray-500"}`}>
          Powered by
        </span>
...
export function PoweredByCorner({ theme = "dark", className = "" }: { theme?: "dark" | "light"; className?: string }) {
  return (
    <div className={`fixed top-4 right-4 z-50 ${className}`}>
      ...
      >
        <span className={`text-xs font-medium tracking-wide uppercase ${theme === "dark" ? "text-gray-500" : "text-gray-400"}`}>
          Powered by
        </span>
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img
