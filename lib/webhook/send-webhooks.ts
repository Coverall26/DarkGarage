...
const publishWebhookEventToQStash = async ({
  ...
  if (!qstash) {
    logger.warn("QStash client not configured - skipping webhook publish", { module: "send-webhooks" });
  ...
  if (!response.messageId) {
    logger.error("Failed to publish webhook event to QStash", { module: "send-webhooks", response: JSON.stringify(response) });
  }
