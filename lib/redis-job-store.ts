...
export class RedisJobStore {
  ...
  async createJob(
    ...
    if (!redis) {
      logger.warn("Redis not available, cannot create job", { module: "redis-job-store" });
      return null;
    }
  ...
  async getJob(jobId: string): Promise<ExportJob | null> {
    ...
    try {
      ...
      logger.error("Error parsing job data", { module: "redis-job-store", error: String(error) });
      return null;
    }
  }
  ...
  async getBlobsForCleanup(
    ...
    for (const item of items) {
      try {
        ...
        logger.error("Error parsing cleanup item", { module: "redis-job-store", error: String(error) });
      }
    }
