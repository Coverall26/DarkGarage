...
export class VercelBlobProvider implements StorageProvider {
  ...
  constructor(config: StorageConfig) {
    ...
    if (!this.encryptionEnabled) {
      logger.warn("Encryption not enabled — Vercel Blob uses public URLs. Without encryption, uploaded data will be publicly accessible. Set STORAGE_ENCRYPTION_KEY to enable server-side encryption.", { module: "vercel-provider" });
    }
  }
  ...
  async get(
    ...
    try {
      ...
      logger.error("Error downloading from Vercel Blob", { module: "vercel-provider", key, error: String(error) });
      return null;
    }
  }
  ...
  async delete(key: string): Promise<boolean> {
    try {
      ...
      logger.error("Error deleting from Vercel Blob", { module: "vercel-provider", key, error: String(error) });
      return false;
    }
  }
  ...
  async list(options?: StorageListOptions): Promise<StorageListResult> {
    try {
      ...
      logger.error("Error listing Vercel Blob objects", { module: "vercel-provider", error: String(error) });
