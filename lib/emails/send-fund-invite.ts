...
export async function sendInviteFundEmail({
  ...
  if (!resend) {
    logger.warn("Resend not initialized — skipping fund invite email", { module: "send-fund-invite" });
    return;
  }
  ...
  try {
    ...
    logger.error("Failed to send fund invite", { module: "send-fund-invite", error: String(error), toEmail, fundName });
    throw error;
  }
}
