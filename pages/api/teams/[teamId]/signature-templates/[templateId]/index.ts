...
async function handleGet(
  ...
  try {
    ...
    try {
      ...
      logger.error("Error getting template file URL", { module: "teams", metadata: { error: (error as Error).message } });
    }
    ...
    reportError(error as Error);
    logger.error("Error fetching template", { module: "teams", metadata: { error: (error as Error).message } });
...
async function handlePut(
  ...
  try {
    ...
    reportError(error as Error);
    logger.error("Error updating template", { module: "teams", metadata: { error: (error as Error).message } });
    return res.status(500).json({ error: "Internal server error" });
  }
}
...
async function handleDelete(
  ...
  try {
    ...
    reportError(error as Error);
    logger.error("Error deleting template", { module: "teams", metadata: { error: (error as Error).message } });
    return res.status(500).json({ error: "Internal server error" });
  }
}
