...
export default function DomainConfiguration({
  ...
  response: { domainJson: Record<string, unknown>; configJson: Record<string, unknown> };
}) {
  const { domainJson, configJson } = response;
  const subdomain = getSubdomain(domainJson.name as string, domainJson.apexName as string);
      ...
      <div className="pt-5">
        <div className="flex justify-start space-x-4">
          <div className="ease border-b-2 border-black pb-1 text-sm text-foreground transition-all duration-150">
            {(configJson?.conflicts as { type: string }[] | undefined)?.some((x: { type: string }) => x.type === "A")
        ...
        <DnsRecord
          ...
          records={(configJson?.conflicts as { name: string; type: string; value: string }[] | undefined)?.map(
            ({
              name,
              type,
              value,
  ...
  if (status === "Unknown Error") {
    return (
      <div className="pt-5">
        <p className="mb-5 text-sm">{(response.domainJson.error as { message: string })?.message}</p>
      </div>
    );
  }
  ...
  const txtVerification =
    status === "Pending Verification"
      ? (domainJson.verification as { type: string; domain: string; value: string }[] | undefined)?.find((x: { type: string }) => x.type === "TXT")
      : undefined;
        ...
        records={[
          {
            ...
            value:
              recordType === "A"
                ? (((configJson?.recommendedIPv4 as Array<{ value: string[] }> | undefined)?.[0]?.value?.[0]) ??
                  "76.76.21.21")
                : (((configJson?.recommendedCNAME as Array<{ value: string }> | undefined)?.[0]?.value) ??
            ...
            ? [
                {
                  ...
                  name: txtVerification.domain.slice(
                    ...
                    txtVerification.domain.length -
                      (domainJson.apexName as string).length -
                      1,
                  ),
