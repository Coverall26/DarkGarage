...
describe("PATCH /api/funds/[fundId]/settings", () => {
  ...
  it("sets capitalCallThreshold to 0 when empty string provided (coerced by Zod)", async () => {
    (prisma.fund.update as jest.Mock).mockResolvedValue({
      ...mockFund,
      capitalCallThreshold: 0,
    });

    const [req, ctx] = makePatchRequest("fund-1", { capitalCallThreshold: "" });
    const res = await PATCH(req, ctx);

    // z.coerce.number() coerces "" to 0, not null
    const updateCall = (prisma.fund.update as jest.Mock).mock.calls[0][0];
    expect(updateCall.data.capitalCallThreshold).toBe(0);
  });
  ...
  it("rejects invalid callFrequency values", async () => {
    ...
    // WEEKLY is not in the Zod enum, so validateBody returns 400 with "Validation failed"
