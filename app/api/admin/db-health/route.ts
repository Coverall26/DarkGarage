...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    for (const tableName of CRITICAL_TABLES) {
      const primaryModel = (primary as any)[tableName]; // Dynamic Prisma delegate access by string key
      const backupModel = (backup as any)[tableName]; // Dynamic Prisma delegate access by string key
