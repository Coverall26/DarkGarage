...
describe("Funding Round Detail API", () => {
  ...
  describe("PUT /funding-rounds/[roundId]", () => {
    ...
    it("parses decimal fields correctly", async () => {
      ...
      const updateCall = (prisma.fundingRound.update as jest.Mock).mock.calls[0][0];
      expect(updateCall.data.amountRaised).toBe(1500000);
      expect(updateCall.data.targetAmount).toBe(3000000);
    });
