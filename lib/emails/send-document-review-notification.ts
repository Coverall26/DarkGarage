...
export async function sendDocumentReviewNotification({
  ...
  try {
    ...
    if (!lpDocument?.investor?.user?.email) {
      logger.warn("No email found for document review notification", { module: "send-document-review-notification", documentId });
      return;
    }
    ...
    logger.error("Failed to send document review notification", { module: "send-document-review-notification", error: String(error), documentId, reviewStatus });
  }
}
