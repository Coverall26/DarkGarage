...
export const deleteFiles = async ({ teamId, data }: DeleteFilesOptions) => {
  ...
  if (data && data.length > 0) {
    logger.warn("Skipping Vercel Blob deletion (not configured)", { module: "delete-team-files-server", fileCount: data.length });
  }
...
const deleteAllFilesFromS3Server = async (teamId: string) => {
  ...
  try {
    ...
    logger.error("Error deleting team files from S3", { module: "delete-team-files-server", teamId, error: String(error) });
  }
};
