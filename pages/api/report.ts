...
export default async function handler(
  ...
  try {
    ...
    logger.error("Error occurred", { module: "report", metadata: { error: (err as Error).message } });
      ...
      Promise.all(
        adminEmails.map((email) =>
          sendEmail({
            ...
            logger.error(`Failed to send abuse report email to ${email}:`, { module: "report", metadata: { error: (err as Error).message } });
          })
        )
      )
    );
