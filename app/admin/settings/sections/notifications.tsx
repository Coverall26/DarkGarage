    ...
    <div className="space-y-4">
      <div>
        ...
        <div className="mt-2 space-y-1">
          <ToggleRow
            label="Investor Onboarding Started"
            description="Notify when a new investor begins the onboarding wizard"
            checked={form.notifyGpLpOnboardingStart}
            onChange={(v) => update({ notifyGpLpOnboardingStart: v })}
          />
          <ToggleRow
            label="New Commitment"
            description="Notify when an investor submits a commitment"
            checked={form.notifyGpCommitment}
            onChange={(v) => update({ notifyGpCommitment: v })}
          />
          <ToggleRow
            label="Wire Proof Uploaded"
            description="Notify when an investor uploads proof of wire transfer"
            checked={form.notifyGpWireUpload}
            onChange={(v) => update({ notifyGpWireUpload: v })}
          />
          <ToggleRow
            label="Investor Inactive Alert"
            description="Notify when an investor has not progressed in onboarding"
            checked={form.notifyGpLpInactive}
            onChange={(v) => update({ notifyGpLpInactive: v })}
          />
          <ToggleRow
            label="External Document Upload"
            description="Notify when an investor uploads a document for review"
