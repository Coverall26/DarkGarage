...
import { captureFromDataroomView } from "@/lib/crm/contact-upsert-job";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { email, password, name, hasConfirmedAgreement, hasConfirmedAccreditation, hasCompletedSignSuiteNda } = data as {
      email: string;
      password: string;
      name?: string;
      hasConfirmedAgreement?: boolean;
      hasConfirmedAccreditation?: boolean;
      hasCompletedSignSuiteNda?: boolean;
    };
    ...
    if (!dataroomSession && !isPreview && !isEmailVerified) {
      ...
      // Check if SignSuite NDA signing is required for visiting the link
      if (link.enableSignSuiteNda && link.signSuiteNdaDocumentId) {
        const ndaRecord = await prisma.ndaSigningRecord.findFirst({
          where: {
            linkId: linkId,
        ...
        if (!ndaRecord) {
          return NextResponse.json(
            { error: "SignSuite NDA signing is required before accessing this dataroom." },
            { status: 400 },
          );
        }
      }
      ...
      if (link.emailAuthenticated && code && !dataroomVerified && !isEmailVerified) {
        ...
        if (isMagicLinkValid) {
          // Magic link verified — create a ViewerSession via verifyCode
          ...
          if (result.verified) {
            hashedVerificationToken = result.sessionToken;
            isEmailVerified = true;
          }
        } else {
          // Verify the 6-digit OTP code (hashed comparison, max 3 attempts)
          const result = await verifyCode({
            email,
            linkId,
            code,
          ...
          if (!result.verified) {
            return NextResponse.json(
              {
                error: result.error,
                resetVerification: result.resetVerification,
              },
              { status: 401 },
            );
          }
      ...
      if (link.emailAuthenticated && token && !dataroomVerified) {
        ...
        // Validate the ViewerSession token for repeat access
        const result = await validateViewerSession({
          token,
          linkId,
          email,
        });
    ...
    if (viewType === "DATAROOM_VIEW") {
      try {
        ...
        // Link NdaSigningRecord to the new dataroom view (if SignSuite NDA was required)
        if (newDataroomView && link.enableSignSuiteNda && (viewer?.email ?? email)) {
          prisma.ndaSigningRecord.updateMany({
            where: {
              linkId: linkId,
              signerEmail: viewer?.email ?? email,
              signedAt: { not: null },
              viewId: null,
            },
            data: { viewId: newDataroomView.id },
    ...
    try {
      ...
      if (!isPreview) {
        ...
        if (!dataroomSession) {
          ...
          // Link NdaSigningRecord to the new dataroom view (if SignSuite NDA was required)
          if (link.enableSignSuiteNda && (viewer?.email ?? email)) {
            prisma.ndaSigningRecord.updateMany({
              where: {
                linkId: linkId,
                signerEmail: viewer?.email ?? email,
