...
describe("Admin Login API", () => {
  ...
  describe("Email Validation", () => {
    ...
    it("should trim whitespace from email", async () => {
      // Zod .email() validation runs before .transform(), so whitespace-padded
      // emails fail validation before trimming occurs — returns 400
      const req = makeRequest({ email: "  admin@example.com  " });
      const res = await POST(req);
  ...
  describe("Security Considerations", () => {
    ...
    it("should handle email injection attempts", async () => {
      // Zod .email() validation rejects emails with newlines/injection chars
      const req = makeRequest({ email: "admin@example.com\nBcc: attacker@evil.com" });
      const res = await POST(req);

      expect(res.status).toBe(400);
      expect(mockIsUserAdminAsync).not.toHaveBeenCalled();
    });
  });
});
