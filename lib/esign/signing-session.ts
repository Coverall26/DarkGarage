...
import { autoFileEnvelopeDocument, type FilingResult } from "@/lib/esign/document-filing-service";
import {
  sendSigningCompletedEmails,
  sendNextSignerEmails,
} from "@/lib/emails/send-esign-notifications";
import { validateFieldValue, type FieldType } from "@/lib/esign/field-types";
...
export async function recordSignerCompletion(
  ...
  // ---- Field Validation ----
  // If field values were submitted, validate them against the document's field definitions
  if (input.fieldValues && Object.keys(input.fieldValues).length > 0) {
    // Find the envelope's associated document and its fields
    ...
    if (envelope?.sourceFile) {
      // Look up the SignatureDocument linked to this envelope via sourceFile
      const signatureDoc = await prisma.signatureDocument.findFirst({
        where: { file: envelope.sourceFile },
        select: { id: true },
      });

      if (signatureDoc) {
        // Fetch all field definitions for this document
        const fieldDefs = await prisma.signatureField.findMany({
          where: { documentId: signatureDoc.id },
          select: {
            id: true,
            type: true,
            label: true,
        ...
        if (fieldDefs.length > 0) {
          const validationErrors: { fieldId: string; label: string; error: string }[] = [];
          ...
          for (const field of fieldDefs) {
            ...
            if (field.options) {
              try {
                ...
                if (Array.isArray(parsed)) {
                  fieldOptions = parsed.map(String);
                }
              } catch {
                // options not parseable — skip options validation
              }
            }
            ...
            const result = validateFieldValue(
              ...
              value,
              {
                ...
                fieldOptions,
                fieldFormat: field.fieldFormat ?? undefined,
          ...
          if (validationErrors.length > 0) {
            const errorMsg = validationErrors
              ...
              .join("; ");
            throw new Error(`Field validation failed: ${errorMsg}`);
          }
        }
      }
    }
  ...
  let filingResult: FilingResult | undefined;
