...
export class S3StorageProvider implements StorageProvider {
  ...
  async get(
    ...
    try {
      ...
      logger.error("Error downloading from S3", { module: "s3-provider", error: String(error) });
      throw error;
    }
  }
  ...
  async getStream(
    ...
    try {
      ...
      logger.error("Error streaming from S3", { module: "s3-provider", error: String(error) });
      throw error;
    }
  }
  ...
  async delete(key: string): Promise<boolean> {
    try {
      ...
      logger.error("Error deleting from S3", { module: "s3-provider", key, error: String(error) });
      return false;
    }
  }
  ...
  async list(options?: StorageListOptions): Promise<StorageListResult> {
    try {
      ...
      logger.error("Error listing S3 objects", { module: "s3-provider", error: String(error) });
  ...
  async getSignedUrl(
    ...
    // AWS SDK getSignedUrl requires a single Command type but command is a union — as any needed
  ...
  async copy(sourceKey: string, destKey: string): Promise<boolean> {
    try {
      ...
      logger.error("Error copying S3 object", { module: "s3-provider", sourceKey, destKey, error: String(error) });
      return false;
    }
  }
}
