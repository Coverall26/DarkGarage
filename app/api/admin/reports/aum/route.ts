...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const aumReports = funds.map((fund) => {
 ...
 */
function calculateDeductions(fund: { createdAt: Date; managementFeePct?: unknown; orgFeePct?: unknown; expenseRatioPct?: unknown }, grossAUM: number) {
  const fundAgeYears =
    (Date.now() - new Date(fund.createdAt).getTime()) /
