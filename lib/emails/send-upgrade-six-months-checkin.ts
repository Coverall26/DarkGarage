/**
 * NOT YET WIRED — Six-month subscription milestone email.
 *
 * Template and send helper are complete but no trigger calls this function.
 * Uses Resend's `scheduledAt` for deferred delivery (195 days from call).
 * Phase 2: Wire into CRM billing webhook — fire on initial subscription creation.
 */
...
export const sendSixMonthMilestoneEmail = async (
  ...
  try {
    ...
    logger.error("Failed to send six-month milestone email", { module: "send-upgrade-six-months-checkin", error: String(e), email: email as string });
  }
};
