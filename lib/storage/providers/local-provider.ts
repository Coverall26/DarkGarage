...
export class LocalStorageProvider implements StorageProvider {
  ...
  async get(
    ...
    try {
      ...
      logger.error("Error reading local file", { module: "local-provider", key, error: String(error) });
      throw error;
    }
  }
  ...
  async delete(key: string): Promise<boolean> {
    try {
      ...
      logger.error("Error deleting local file", { module: "local-provider", key, error: String(error) });
      return false;
    }
  }
  ...
  async list(options?: StorageListOptions): Promise<StorageListResult> {
    try {
      ...
      logger.error("Error listing local files", { module: "local-provider", error: String(error) });
  ...
  async copy(sourceKey: string, destKey: string): Promise<boolean> {
    try {
      ...
      logger.error("Error copying local file", { module: "local-provider", sourceKey, destKey, error: String(error) });
      return false;
    }
  }
}
