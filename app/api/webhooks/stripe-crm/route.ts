...
import { provisionModulesForTier } from "@/lib/modules/provision-engine";
...
async function handleCheckoutCompleted(event: Stripe.Event, stripe: Stripe) {
  ...
  if (planInfo.slug === "AI_CRM") {
    ...
    // Provision product modules for the new tier
    provisionModulesForTier(orgId, planInfo.slug === "CRM_PRO" ? "PRO" : planInfo.slug as "FREE" | "PRO" | "BUSINESS" | "FUNDROOM").catch(
      (e) => reportError(e as Error),
    );
  }
...
async function handleSubscriptionUpdated(event: Stripe.Event) {
  ...
  if (planInfo.slug === "AI_CRM") {
    ...
    // Re-provision modules for the updated tier
    provisionModulesForTier(orgId, planInfo.slug === "CRM_PRO" ? "PRO" : planInfo.slug as "FREE" | "PRO" | "BUSINESS" | "FUNDROOM").catch(
      (e) => reportError(e as Error),
    );
  }
...
async function handleSubscriptionDeleted(event: Stripe.Event) {
  ...
  if (isAddon) {
    ...
    // Base CRM plan cancelled → revert to FREE, keep all data
    await prisma.organization.update({
      where: { id: orgId },
      data: {
    ...
    // Re-provision modules to FREE tier limits (data is preserved, limits reduced)
    provisionModulesForTier(orgId, "FREE").catch(
      (e) => reportError(e as Error),
    );
  }
...
async function handleSubscriptionCreated(event: Stripe.Event) {
  ...
  if (planInfo.slug === "AI_CRM") {
    ...
    // Provision modules for the new tier
    provisionModulesForTier(orgId, planInfo.slug === "CRM_PRO" ? "PRO" : planInfo.slug as "FREE" | "PRO" | "BUSINESS" | "FUNDROOM").catch(
      (e) => reportError(e as Error),
    );
  }
