...
function getEncryptionKey(): string {
  ...
  if (!process.env.PLAID_TOKEN_ENCRYPTION_KEY && process.env.NODE_ENV === 'production') {
    logger.warn("Using NEXTAUTH_SECRET as Plaid encryption fallback. Set PLAID_TOKEN_ENCRYPTION_KEY for production.", { module: "plaid" });
  }
...
export async function verifyWebhookSignature(body: string, signedJwt: string): Promise<boolean> {
  if (!signedJwt) {
    logger.error("Plaid webhook: Missing Plaid-Verification header", { module: "plaid" });
    return false;
  }
  ...
  try {
    ...
    if (!headerB64) {
      logger.error("Plaid webhook: Invalid JWT format", { module: "plaid" });
      return false;
    }
    ...
    if (!keyId) {
      logger.error("Plaid webhook: Missing key ID in JWT header", { module: "plaid" });
      return false;
    }
    ...
    if (payload.request_body_sha256 !== requestBodyHash) {
      logger.error("Plaid webhook: Body hash mismatch", { module: "plaid" });
      return false;
    }

    return true;
  } catch (error) {
    logger.error("Plaid webhook verification error", { module: "plaid", error: String(error) });
    return false;
  }
}
