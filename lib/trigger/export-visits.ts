...
async function exportDocumentVisits(
  ...
  const uniqueCustomFields = collectUniqueCustomFields(views as unknown as ViewWithCustomFields[]);
  ...
  for (let i = 0; i < views.length; i++) {
    ...
    if (!isProPlan) {
      ...
      rowData.push(...extractCustomFieldValues(view as unknown as ViewWithCustomFields, uniqueCustomFields));
    }
...
/** Shape of a custom field entry in a view's customFieldResponse.data array */
...
/** View record with optional customFieldResponse data */
interface ViewWithCustomFields {
  customFieldResponse?: { data?: CustomFieldEntry[] } | null;
  [key: string]: unknown;
}

// Helper function to extract all unique custom fields from views
function collectUniqueCustomFields(
  views: ViewWithCustomFields[],
  ...
  views.forEach((view, index) => {
    try {
      if (
        ...
        view.customFieldResponse.data.forEach((field: CustomFieldEntry) => {
...
async function exportDataroomVisits(
  ...
  const uniqueCustomFields = collectUniqueCustomFields(dataroomViews as unknown as ViewWithCustomFields[]);
  ...
  exportData.forEach((view) => {
    if (view.documentViews.length === 0) {
      ...
      rowData.push(
        ...extractCustomFieldValues((dataroomView ?? {}) as ViewWithCustomFields, uniqueCustomFields),
      );
