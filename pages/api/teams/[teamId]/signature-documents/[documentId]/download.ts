...
export default async function handler(
  ...
  try {
      ...
      if (field.type === "SIGNATURE") {
        ...
        if (recipient?.signatureImage) {
          try {
            ...
            logger.error("Failed to embed signature", { module: "teams", metadata: { error: (err as Error).message } });
          }
        }
      } else if (field.value) {
    ...
    logger.error("Error downloading signed document", { module: "teams", metadata: { error: (error as Error).message } });
    return res.status(500).json({ error: "Internal server error" });
  }
}
