...
async function logRateLimitViolation(ip: string, endpoint: string) {
  try {
    ...
    logger.error("Failed to log rate limit violation", { module: "security", error: String(error) });
  }
}
...
export const signatureRateLimiter = createRateLimiter({
  ...
  onLimitReached: (id, endpoint) => {
    logger.warn("Signature rate limit exceeded", { module: "security", identifier: id, endpoint });
  },
});
...
export const authRateLimiter = createRateLimiter({
  ...
  keyPrefix: "rl:auth",
  onLimitReached: (id, endpoint) => {
    logger.warn("Auth rate limit exceeded", { module: "security", identifier: id, endpoint });
  },
});
...
export const strictRateLimiter = createRateLimiter({
  ...
  keyPrefix: "rl:strict",
  onLimitReached: (id, endpoint) => {
    logger.error("Strict rate limit exceeded", { module: "security", identifier: id, endpoint });
  },
});
...
export const uploadRateLimiter = createRateLimiter({
  ...
  keyPrefix: "rl:upload",
  onLimitReached: (id, endpoint) => {
    logger.warn("Upload rate limit exceeded", { module: "security", identifier: id, endpoint });
  },
});
...
export const mfaVerifyRateLimiter = createRateLimiter({
  ...
  onLimitReached: (id, endpoint) => {
    logger.error("MFA brute-force attempt", { module: "security", identifier: id, endpoint });
  },
});
