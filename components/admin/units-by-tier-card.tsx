...
export function UnitsByTierCard({ fundId, teamId }: UnitsByTierCardProps) {
  ...
  const fetchTiers = useCallback(async (silent = false) => {
    ...
    try {
      const res = await fetch(`/api/admin/fund/${fundId}/pricing-tiers`);
      if (res.ok) {
      ...
      logger.error("Error fetching pricing tiers", { module: "units-by-tier-card", metadata: { error: (error as Error).message } });
    } finally {
      setIsLoading(false);
