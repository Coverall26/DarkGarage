...
const trustedConnectDomains = [
  ...
  // S3 storage — scoped to s3 service domains
  ...
  // WebSocket support (SSE, live updates) — scoped to platform domains
  "wss://*.fundroom.ai",
  "wss://fundroom.ai",
  ...(isDev ? ["wss:"] as string[] : []),
...
const trustedImageDomains = [
  ...
  "https://36so9a8uzykxknsu.public.blob.vercel-storage.com",
  "https://blob.vercel-storage.com",
  // S3 document thumbnails and images — scoped to s3 service domains
  "https://*.s3.amazonaws.com",
  "https://*.s3.*.amazonaws.com",
  "https://s3.amazonaws.com",
  "https://s3.*.amazonaws.com",
  // R2 document thumbnails and images
  "https://*.r2.cloudflarestorage.com",
  ...dynamicImageHosts,
].join(" ");
...
export function getFrameAncestors(path: string): string {
  ...
  if (path.includes("/embed")) {
    // Require explicit allowlist via CSP_EMBED_ALLOWED_ORIGINS or CSP_EMBED_ALLOW_ALL=true
    // Never default to 'https:' — attacker-controlled origins could frame the page
    if (embedAllowedOrigins) {
