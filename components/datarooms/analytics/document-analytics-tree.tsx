...
import {
  TDataroomStatsData,
  useDataroomStats,
} from "@/lib/swr/use-dataroom-stats";
...
const buildTree = (
  ...
  statsData: TDataroomStatsData | undefined,
  ...
  const getAnalytics = (id: string, documentId?: string) => {
    ...
    const views =
      statsData?.documentViews.filter(
        (view) => view.documentId === id || view.documentId === documentId,
      ).length || 0;
    ...
    const downloads =
      statsData?.documentViews.filter(
        (view) =>
          (view.documentId === id || view.documentId === documentId) &&
          view.downloadedAt,
      ).length || 0;
