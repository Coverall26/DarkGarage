...
export class VerifyInvestorKycProvider implements KycProvider {
  ...
  async startVerification(input: KycStartInput): Promise<KycSession> {
    ...
    if (!response.ok) {
      ...
      logger.error("Verify Investor API error", { module: "verify-investor", error: String(error) });
      throw new Error(
  ...
  verifyWebhookSignature(payload: string, signature: string): boolean {
    if (!this.config.webhookSecret) {
      logger.warn("Verify Investor webhook secret not configured", { module: "verify-investor" });
      return false;
    }
