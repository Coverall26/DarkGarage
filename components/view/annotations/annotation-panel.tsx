...
export function AnnotationPanel({
  ...
  const renderContent = (content: TipTapContent | string | null | undefined) => {
          ...
          {content.content.map((node: TipTapNode, index: number) => {
            if (node.type === "paragraph") {
              return (
                ...
                >
                  {node.content?.map((textNode: TipTapNode, textIndex: number) => {
                    if (textNode.type === "text") {
                      let text: React.ReactNode = textNode.text;
                      // Apply formatting if marks exist
                      if (textNode.marks) {
                        textNode.marks.forEach((mark: { type: string }) => {
                          if (mark.type === "bold") {
                            text = (
              ...
              return (
                ...
                >
                  {node.content?.map((listItem: TipTapNode, listIndex: number) => (
