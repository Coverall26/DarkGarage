...
export class ParallelMarketsKycProvider implements KycProvider {
  ...
  async startVerification(input: KycStartInput): Promise<KycSession> {
    ...
    if (!response.ok) {
      ...
      logger.error("Parallel Markets API error", { module: "parallel-markets", error: String(error) });
      throw new Error(
  ...
  verifyWebhookSignature(payload: string, signature: string): boolean {
    if (!this.config.webhookSecret) {
      logger.warn("Parallel Markets webhook secret not configured", { module: "parallel-markets" });
      return false;
    }
