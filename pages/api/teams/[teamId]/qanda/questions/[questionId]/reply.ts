    ...
    try {
      ...
      if (question.viewerEmail) {
        try {
          ...
          logger.error("Failed to send reply notification", { module: "teams", metadata: { error: (emailError as Error).message } });
        }
      }
      ...
      logger.error("Error creating reply", { module: "teams", metadata: { error: (error as Error).message } });
