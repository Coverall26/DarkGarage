  ...
  try {
    ...
    if (!isTeamMember) {
      ...
      if (link.enableSignSuiteNda && link.signSuiteNdaDocumentId) {
        ...
        if (!ndaRecord) {
          return NextResponse.json(
            { error: "SignSuite NDA signing is required before accessing this document." },
            { status: 400 },
          );
        }
      }
      ...
      if (link.emailAuthenticated && code) {
        ...
        if (isMagicLinkValid) {
          // Magic link verified — create session via viewer-auth
          ...
          // Verify OTP code via hashed VerificationCode model
          const result = await verifyCode({
            email,
            linkId,
            code,
      ...
      if (link.emailAuthenticated && token) {
        ...
        // Validate existing ViewerSession token for repeat access
    ...
    try {
      ...
      if (!isPreview) {
        ...
        // Link NdaSigningRecord to the new view (if SignSuite NDA was required)
        if (newView && link.enableSignSuiteNda && email) {
