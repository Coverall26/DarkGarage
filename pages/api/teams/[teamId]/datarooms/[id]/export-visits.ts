...
export default async function handler(
  ...
  if (req.method === "GET") {
    ...
    try {
      ...
      logger.error("Error fetching existing exports", { module: "teams", metadata: { error: (error as Error).message } });
      return res
        .status(500)
  ...
  try {
    ...
    reportError(error as Error);
    logger.error("Error creating export job", { module: "teams", metadata: { error: (error as Error).message } });
