...
export interface CreateEnvelopeInput {
  ...
  sourceModule?: "DATAROOM" | "SIGNSUITE" | "DOCROOMS" | "PIPELINE_IQ_LITE" | "PIPELINE_IQ" | "FUNDROOM";
}
...
export async function sendEnvelope(
  ...
  const updated = await prisma.$transaction(async (tx) => {
    // Update envelope status (clear scheduledSendAt if it was scheduled)
...
export async function scheduleEnvelope(
  envelopeId: string,
  userId: string,
  scheduledSendAt: Date
): Promise<Envelope> {
  ...
  if (envelope.status !== "DRAFT" && envelope.status !== "PREPARING") {
    throw new Error(`Cannot schedule envelope in ${envelope.status} status`);
  }
...
// ============================================================================
// Cancel Scheduled Envelope
// ============================================================================

export async function cancelScheduledEnvelope(
  envelopeId: string,
  userId: string
): Promise<Envelope> {
  const envelope = await prisma.envelope.findUnique({
  ...
  if (envelope.status !== "SCHEDULED") {
    throw new Error(`Cannot cancel schedule for envelope in ${envelope.status} status`);
  }
  ...
  logAuditEvent({
    eventType: "ENVELOPE_SCHEDULE_CANCELLED",
    userId,
    teamId: envelope.teamId,
    resourceType: "Envelope",
    resourceId: envelopeId,
