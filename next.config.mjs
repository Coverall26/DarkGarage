...
const nextConfig = {
  ...
  async redirects() {
    return [
      ...
      // Legacy esign → signsuite (admin/esign directory removed)
      { source: "/admin/esign", destination: "/admin/signsuite", permanent: true },
      { source: "/admin/esign/:path*", destination: "/admin/signsuite", permanent: true },
      // Orphaned subscriptions route → signsuite (H2 cleanup, dead code removed)
      { source: "/admin/subscriptions/new", destination: "/admin/signsuite", permanent: true },
      // Orphaned entities route → fund management (H2 cleanup, entity page deleted)
      { source: "/admin/entities", destination: "/admin/fund", permanent: true },
      { source: "/admin/entities/:path*", destination: "/admin/fund", permanent: true },
      // Consolidated fund routes: /admin/funds → /admin/fund (H1 cleanup)
      { source: "/admin/funds", destination: "/admin/fund", permanent: true },
      { source: "/admin/funds/:path*", destination: "/admin/fund/:path*", permanent: true },
      // Orphaned settings/fund sub-route → settings center fund tab (M8 cleanup)
      { source: "/admin/settings/fund", destination: "/admin/settings?tab=fundInvestor", permanent: true },
      // Legacy fund-settings API → consolidated admin endpoints (H1 cleanup)
      { source: "/api/fund-settings/funds", destination: "/api/admin/fund/list", permanent: true },
      { source: "/api/fund-settings/update", destination: "/api/admin/fund/threshold-settings", permanent: true },
      { source: "/api/fund-settings/:fundId", destination: "/api/admin/fund/:fundId/threshold-settings", permanent: true },
    ];
  },
  async headers() {
    return [
