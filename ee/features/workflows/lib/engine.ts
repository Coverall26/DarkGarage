...
export class WorkflowEngine {
  ...
  async execute(
    ...
    try {
      ...
      try {
        ...
        for (const step of workflow.steps) {
          ...
          try {
            ...
            logger.error(`Error executing step ${step.id}`, { module: "workflows", metadata: { error: (stepError as Error).message } });
          }
        }
      ...
      logger.error("Workflow execution error", { module: "workflows", metadata: { error: (error as Error).message } });
      return {
        success: false,
        error:
