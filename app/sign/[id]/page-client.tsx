...
export default function SignatureDocumentDetail() {
  ...
  const handleSaveAsTemplate = async () => {
    ...
    try {
      ...
      const templateFields = docData.fields?.map((f: { type: string; pageNumber: number; x: number; y: number; width: number; height: number; required: boolean; placeholder?: string; recipientId: string }, index: number) => {
        const recipientIndex = docData.recipients?.findIndex((r: { id: string }) => r.id === f.recipientId);
        return {
          type: f.type,
      ...
      const defaultRecipients = docData.recipients?.map((r: { name: string; email: string; role: string; signingOrder: number }) => ({
        name: r.name,
        email: r.email,
        role: r.role,
