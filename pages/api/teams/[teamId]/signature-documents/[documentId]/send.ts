...
export async function sendToNextSigners(
  ...
  if (!baseUrl) {
    logger.error("NEXTAUTH_URL not configured, cannot send signing emails", { module: "teams" });
    return;
  }
...
export default async function handler(
  ...
  try {
    ...
    logger.error("Error sending document", { module: "teams", metadata: { error: (error as Error).message } });
