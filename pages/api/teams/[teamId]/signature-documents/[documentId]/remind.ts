  ...
  try {
    ...
    for (const recipient of recipientsToRemind) {
      ...
      try {
        ...
        logger.error(`Failed to send reminder to ${recipient.email}:`, { module: "teams", metadata: { error: (emailError as Error).message } });
    ...
    logger.error("Error sending reminders", { module: "teams", metadata: { error: (error as Error).message } });
