...
export async function getTeamAdminEmails(teamId: string): Promise<string[]> {
  try {
    ...
    logger.error("Error fetching team admin emails", { module: "admins", teamId, error: String(error) });
    return [DEFAULT_ADMIN_EMAIL];
  }
}
...
export async function getAllAdminEmails(): Promise<string[]> {
  try {
    ...
    logger.error("Error fetching all admin emails", { module: "admins", error: String(error) });
    return [DEFAULT_ADMIN_EMAIL];
  }
}
...
export async function isUserAdminAsync(email: string): Promise<boolean> {
  ...
  try {
    ...
    logger.error("Error checking admin status", { module: "admins", error: String(error) });
...
export async function isTeamAdmin(email: string, teamId: string): Promise<boolean> {
  try {
    ...
    logger.error("Error checking team admin status", { module: "admins", teamId, error: String(error) });
    return false;
  }
}
