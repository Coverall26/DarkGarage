...
export default async function handler(
  ...
  if (req.method === "GET") {
    try {
      ...
      logger.error("Error fetching export job", { module: "teams", metadata: { error: (error as Error).message } });
    ...
    try {
      ...
      if (exportJob.triggerRunId) {
        try {
          ...
          logger.error("Failed to cancel trigger run", { module: "teams", metadata: { error: (error as Error).message } });
      ...
      logger.error("Error cancelling export job", { module: "teams", metadata: { error: (error as Error).message } });
      return res.status(500).json({ error: "Internal server error" });
    }
  }
  ...
  if (req.method === "DELETE") {
    try {
      ...
      logger.error("Error deleting export job", { module: "teams", metadata: { error: (error as Error).message } });
      return res.status(500).json({ error: "Internal server error" });
    }
  }
