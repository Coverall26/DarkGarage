...
export async function getUploadTransport(): Promise<string> {
  ...
  if (typeof window === 'undefined') {
    ...
    if (baseUrl) {
      try {
        ...
        logger.error("Failed to fetch upload config on server", { module: "put-file", error: String(error) });
      }
    }
  ...
  try {
    ...
    logger.error("Failed to fetch upload config", { module: "put-file", error: String(error) });
  }
...
export const putFile = async ({
  ...
  const { type, data, numPages, fileSize } = await match(uploadTransport)
    ...
    .otherwise(() => {
      logger.error("Unknown upload transport", { module: "put-file", transport: uploadTransport });
      return {
        type: null,
        data: null,
        numPages: undefined,
...
const putFileMultipart = async ({
  ...
  try {
    ...
    logger.error("Multipart upload failed, falling back to single upload", { module: "put-file", teamId, docId, error: String(error) });
    return await putFileSingle({ file, teamId, docId });
  }
};
