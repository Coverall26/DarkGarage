...
export const sendWelcomeEmail = async (params: CreateUserEmailProps) => {
  ...
  if (isAdmin) {
    ...
    try {
      ...
      logger.error("Failed to send admin welcome email", { module: "send-welcome", error: String(e), email: email as string });
    }
    return;
  }
  ...
  try {
    ...
    logger.error("Failed to send viewer welcome email", { module: "send-welcome", error: String(e), email: email as string });
  }
};
