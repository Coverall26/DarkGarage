...
export function AddDataroomModal({
  ...
  const [generatedFolders, setGeneratedFolders] = useState<GeneratedFolder[] | null>(null);
  ...
  const generateFolderPath = (folder: GeneratedFolder, parentPath: string = ""): string => {
  ...
  const toggleFolderSelection = (path: string, folder: GeneratedFolder) => {
    const newSelected = new Set(selectedFolderPaths);

    if (newSelected.has(path)) {
      // If unchecking, remove this folder and all its subfolders
      newSelected.delete(path);
      const removeSubfolders = (f: GeneratedFolder, currentPath: string) => {
        if (f.subfolders) {
          f.subfolders.forEach((sub: GeneratedFolder) => {
  ...
  // Filter folders based on selection
  const filterSelectedFolders = (folders: GeneratedFolder[], parentPath: string = ""): GeneratedFolder[] => {
    return folders
      .map((folder) => {
  ...
  const initializeFolderSelection = (folders: GeneratedFolder[], parentPath: string = "") => {
    const paths = new Set<string>();
    const collectPaths = (f: GeneratedFolder[], currentPath: string = "") => {
      f.forEach((folder) => {
        const folderPath = generateFolderPath(folder, currentPath);
