  ...
  try {
    ...
    const validEvents = events.filter((event) => {
      ...
      if (!event || typeof event.event_type !== "string" || !event.view_id) {
        logger.warn("Invalid event structure", { module: "teams", metadata: { event: JSON.stringify(event) } });
        return false;
      }
      ...
      if (
        ...
        logger.warn("Invalid time properties", { module: "teams", metadata: { event: JSON.stringify(event) } });
        return false;
      }
    ...
    validEvents.forEach((event) => {
      ...
      for (
        ...
        if (!stats) {
          logger.warn(`No stats found for time ${t}, skipping`, { module: "teams" });
          continue;
        }
    ...
    logger.error("Error calculating analytics", { module: "teams", metadata: { error: (error as Error).message } });
    throw error;
  }
}
...
export default async function handle(
  ...
  try {
    ...
    reportError(error as Error);
    logger.error("Error in video-analytics", { module: "teams", metadata: { error: error instanceof Error ? error.message : "Unknown error" } });
