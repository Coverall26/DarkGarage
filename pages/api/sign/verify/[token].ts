...
export default async function handler(
  ...
  try {
    ...
    try {
      ...
      logger.error("Failed to fetch document for verification", { module: "sign", metadata: { error: (error as Error).message } });
    ...
    const consentRecord = recipient.consentRecord as
      { consentedAt?: string; consentVersion?: string; consentType?: string } | null;
    ...
    reportError(error as Error);
    logger.error("Signature verification error", { module: "sign", metadata: { error: (error as Error).message } });
    return res.status(500).json({ 
      verified: false,
