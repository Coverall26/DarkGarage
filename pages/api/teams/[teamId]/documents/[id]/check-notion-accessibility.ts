  ...
  try {
    ...
    try {
      ...
      try {
        ...
        logger.error("Error checking Notion accessibility", { module: "teams", metadata: { error: (error as Error).message } });
        return res.status(200).json({
          isAccessible: false,
    ...
    reportError(error as Error);
    logger.error("Error", { module: "teams", metadata: { error: (error as Error).message } });
