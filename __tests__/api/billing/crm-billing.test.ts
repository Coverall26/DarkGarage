...
describe("CRM Products (lib/stripe/crm-products)", () => {
  describe("CRM_PLANS", () => {
    ...
    it("CRM_PRO plan has $29/mo pricing", () => {
      expect(CRM_PLANS.CRM_PRO.monthlyPriceDollars).toBe(29);
      expect(CRM_PLANS.CRM_PRO.yearlyPriceDollars).toBe(23);
      expect(CRM_PLANS.CRM_PRO.price.monthly.amount).toBe(2900);
    });
  ...
  describe("getUpgradePath", () => {
    ...
    it("CRM_PRO upgrades to BUSINESS", () => {
      expect(getUpgradePath("CRM_PRO")).toBe("BUSINESS");
    });

    it("BUSINESS upgrades to FUNDROOM", () => {
      expect(getUpgradePath("BUSINESS")).toBe("FUNDROOM");
    });
  ...
  describe("isDowngrade", () => {
    ...
    it("BUSINESS to CRM_PRO is a downgrade", () => {
      expect(isDowngrade("BUSINESS", "CRM_PRO")).toBe(true);
    });

    it("CRM_PRO to BUSINESS is NOT a downgrade", () => {
      expect(isDowngrade("CRM_PRO", "BUSINESS")).toBe(false);
    });

    it("FUNDROOM to BUSINESS is a downgrade", () => {
      expect(isDowngrade("FUNDROOM", "BUSINESS")).toBe(true);
    });
...
describe("CRM Billing Utilities (lib/billing/crm-billing)", () => {
  ...
  describe("getCrmBillingStatus", () => {
    ...
    it("returns billing status for CRM_PRO org with AI add-on", async () => {
      ...
      // $29 base + $49 AI add-on = $78/mo
      expect(status.monthlyPrice).toBe(78);
