...
async function insertWatermark(
  ...
  try {
    ...
    reportError(error as Error);
    logger.warn("Font encoding error", { module: "mupdf", metadata: { error: (error as Error).message } });
  ...
  if (config.isTiled) {
    ...
    for (let i = 0; i < maxTilesPerRow; i++) {
      for (let j = 0; j < maxTilesPerColumn; j++) {
        ...
        try {
          ...
          reportError(error as Error);
          logger.error("Error drawing tiled watermark text", { module: "mupdf", metadata: { error: (error as Error).message } });
          throw new Error(
    ...
    try {
      ...
      reportError(error as Error);
      logger.error("Error drawing positioned watermark text", { module: "mupdf", metadata: { error: (error as Error).message } });
      throw new Error(
        `Failed to apply watermark to page ${pageIndex + 1}`,
      );
    }
        ...
        batchPromises.push(
          insertWatermark(pdfDoc, watermarkConfig, viewerData, i, font).catch(
            (error) => {
              ...
              reportError(error as Error);
              logger.error(`Error watermarking page ${i + 1}`, { module: "mupdf", metadata: { error: errMsg } });
