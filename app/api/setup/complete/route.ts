...
import { SetupCompleteSchema } from "@/lib/validations/setup";
...
export async function POST(req: NextRequest) {
  ...
  try {
    const parsed = await validateBody(req, SetupCompleteSchema);
    ...
    // Accepts `unknown` because Zod-preprocessed fields may be number | null | undefined
    const parseNum = (val: unknown): number | null => {
      if (val === undefined || val === null || val === "") return null;
      const n = typeof val === "string" ? Number(String(val).replace(/[^0-9.]/g, "")) : Number(val);
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const org = await tx.organization.create({
        data: {
          ...
          productMode: data.raiseMode === "GP_FUND" ? "FUND"
            : data.raiseMode === "STARTUP" ? "STARTUP"
            : null,
          relatedPersons: Array.isArray(data.relatedPersons) && data.relatedPersons.length > 0
            ? (data.relatedPersons as Prisma.InputJsonValue)
            : Prisma.JsonNull,
