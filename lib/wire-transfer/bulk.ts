  ...
  for (const investmentId of input.investmentIds) {
    try {
      ...
      import("@/lib/emails/send-proof-notifications")
        ...
        .catch((err) =>
          logger.warn("Email notification failed during bulk verify", { module: "wire-transfer-bulk", investmentId, error: String(err) }),
        );
  ...
  for (const investmentId of input.investmentIds) {
    try {
      ...
      import("@/lib/emails/send-proof-notifications")
        ...
        .catch((err) =>
          logger.warn("Email notification failed during bulk reject", { module: "wire-transfer-bulk", investmentId, error: String(err) }),
        );
