  ...
  describe("POST /api/outreach/sequences", () => {
    ...
    test("creates sequence with steps", async () => {
      const req = makeRequest("POST", {
        ...
        steps: [
          { order: 0, type: "template", templateId: "tpl-1", delayDays: 0 },
          { order: 1, type: "ai", aiPrompt: "Write a follow-up", delayDays: 3 },
        ],
      });
      const res = await handler(req);
    ...
    test("rejects empty name", async () => {
      const req = makeRequest("POST", { name: "", steps: [{ order: 0, type: "ai", aiPrompt: "hi" }] });
      const res = await handler(req);
      expect(res.status).toBe(400);
    });
