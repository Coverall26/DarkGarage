...
import { checkModuleLimit } from "@/lib/middleware/module-access";
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Query contacts, filtered total, and unfiltered usage count in parallel
    const [contacts, total, contactCount] = await Promise.all([
      prisma.contact.findMany({
        ...
        include: {
          ...
          ...(tier.hasLpOnboarding
            ...
            : {}),
        } as any, // Prisma conditional include spread causes Exact<> type constraint & stack depth issues
        orderBy: { [orderField]: sortDir },
        skip: (page - 1) * limit,
        take: limit,
      }),
      prisma.contact.count({ where }),
      // Unfiltered count for tier usage display
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Module-level contact limit check (RAISE_CRM MAX_CONTACTS)
    const currentContactCount = await prisma.contact.count({ where: { teamId } });
    const moduleLimitBlocked = await checkModuleLimit(orgId, "RAISE_CRM", "MAX_CONTACTS", currentContactCount);
    if (moduleLimitBlocked) return moduleLimitBlocked;

    // Tier-level contact limit check (existing)
