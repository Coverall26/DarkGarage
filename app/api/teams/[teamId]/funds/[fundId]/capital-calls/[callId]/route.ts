...
import type { Prisma, CapitalCallStatus } from "@prisma/client";
...
function serializeCall(call: Record<string, unknown> & { amount?: { toNumber?: () => number }; proRataPercentage?: { toNumber?: () => number } | null; responses?: Array<Record<string, unknown> & { amountDue?: { toNumber?: () => number }; amountPaid?: { toNumber?: () => number } }> }) {
  return {
    ...call,
...
export async function PATCH(
  ...
  try {
    ...
    const updateData: Prisma.CapitalCallUpdateInput = {};
    ...
    if (body.status !== undefined) updateData.status = body.status as CapitalCallStatus;
