...
import { TrendingUp, ArrowRight, Eye, Settings, BarChart3 } from "lucide-react";
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <CardTitle className="text-base flex items-center gap-2">
              <div className="h-7 w-7 rounded-lg flex items-center justify-center" style={{ backgroundColor: SUITE_COLORS.dataroom }}>
    ...
    <Card className="shadow-sm overflow-hidden">
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="text-base flex items-center gap-2">
              <div className="h-7 w-7 rounded-lg flex items-center justify-center" style={{ backgroundColor: SUITE_COLORS.raiseroom }}>
      ...
      <CardContent>
        ...
        <div className="relative h-8 bg-muted/80 rounded-full overflow-hidden">
          <div
            ...
            style={{ backgroundColor: `${SUITE_COLORS.raiseroom}33`, width: `${committedPercent}%` }}
          />
          <div
            className="absolute inset-y-0 left-0 rounded-full transition-all duration-1000 ease-out"
            style={{
              width: `${fundedPercent}%`,
              background: `linear-gradient(90deg, #0891B2 0%, ${SUITE_COLORS.raiseroom} 50%, #22D3EE 100%)`,
            }}
          >
            <div
        ...
        <div className="flex justify-between mt-2.5 text-xs text-muted-foreground">
          <div className="flex items-center gap-4">
            <span className="flex items-center gap-1.5">
              <span className="h-2 w-2 rounded-full" style={{ backgroundColor: SUITE_COLORS.raiseroom }} />
              Funded
              <span className="font-mono tabular-nums">{formatCurrency(raise.totalFunded)}</span>
            </span>
            <span className="flex items-center gap-1.5">
              <span className="h-2 w-2 rounded-full" style={{ backgroundColor: `${SUITE_COLORS.raiseroom}40` }} />
              Committed
                ...
                <div key={fund.id}>
                  <div className="flex justify-between text-sm mb-1.5">
                    <span className="flex items-center gap-1.5">
                      <Link
                        ...
                        style={{ ["--hover-color" as string]: SUITE_COLORS.raiseroom }}
                        onMouseEnter={(e) => (e.currentTarget.style.color = SUITE_COLORS.raiseroom)}
                    ...
                    <span className="text-muted-foreground text-xs font-mono tabular-nums">
                      ...
                      <span className="ml-1.5 text-muted-foreground/50">
                        &middot; {fund.investorCount} Investor{fund.investorCount !== 1 ? "s" : ""}
                      </span>
                    </span>
                  </div>
                  <div className="h-1.5 bg-muted rounded-full overflow-hidden">
                    <div
                      ...
                      style={{ backgroundColor: SUITE_COLORS.raiseroom, width: `${fPercent}%` }}
                    />
                  </div>
                </div>
              );
