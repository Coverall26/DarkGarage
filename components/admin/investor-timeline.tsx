...
export function InvestorTimeline({
  ...
  const fetchTimeline = useCallback(async () => {
    try {
      ...
      logger.error("Failed to fetch timeline", { module: "investor-timeline", metadata: { error: (error as Error).message } });
    } finally {
      setLoading(false);
    }
  ...
  const handleSendReply = async () => {
    ...
    try {
      ...
      logger.error("Failed to send reply", { module: "investor-timeline", metadata: { error: (error as Error).message } });
    } finally {
      setSending(false);
    }
  };
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {event.type === "note" && (
                            <Badge variant={event.isFromInvestor ? "default" : "secondary"}>
                              {event.isFromInvestor ? "From Investor" : "From GP"}
                            </Badge>
                          )}
                        </div>
                        {event.investorName && (
