...
interface FundDetailsTransaction {
  id: string;
  type: string;
  description?: string;
...
interface FundDetailsData {
  ...
  funds: FundDetailsFund[];
  pendingCapitalCalls: Array<Record<string, unknown>>;
  recentTransactions: FundDetailsTransaction[];
  documents: FundDetailsDocument[];
  notes: FundDetailsNote[];
...
export default function LPDashboardClient() {
  ...
  const fetchFundDetails = useCallback(async (silent = false) => {
    ...
    try {
      ...
      logger.error("Error fetching fund details", { module: "lp-dashboard", metadata: { error: (error as Error).message } });
      if (!silent) {
  ...
  const fetchSubscriptionStatus = useCallback(async () => {
    try {
      ...
      logger.error("Error fetching subscription status", { module: "lp-dashboard", metadata: { error: (error as Error).message } });
    }
  }, [investor?.ndaSigned]);
  ...
  const handleProcessPayment = async () => {
    ...
    try {
      ...
      logger.error("Payment error", { module: "lp-dashboard", metadata: { error: (error as Error).message } });
  ...
  useEffect(() => {
    if (investor?.ndaSigned && investor?.accreditationStatus !== "PENDING") {
      fetch("/api/lp/staged-commitment")
        ...
        .catch((e) => logger.error("Failed to load staged commitment info", { module: "lp-dashboard", metadata: { error: (e as Error).message } }));
    }
  ...
  useEffect(() => {
    ...
    if (investor?.ndaSigned && investor?.accreditationStatus === "VERIFIED") {
      fetch("/api/lp/offering-documents")
        ...
        .catch((e) => logger.error("Failed to load offering documents", { module: "lp-dashboard", metadata: { error: (e as Error).message } }));
    }
  ...
  const fetchProfile = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        logger.error("Error fetching bank status", { module: "lp-dashboard", metadata: { error: (e as Error).message } });
      }
      ...
      logger.error("Error fetching investor profile", { module: "lp-dashboard", metadata: { error: (error as Error).message } });
  ...
  const fetchSignatures = useCallback(async () => {
    ...
    try {
      ...
      logger.error("Error fetching signatures", { module: "lp-dashboard", metadata: { error: (error as Error).message } });
  ...
  const fetchDocs = useCallback(async () => {
    ...
    try {
      ...
      logger.error("Error fetching documents", { module: "lp-dashboard", metadata: { error: (error as Error).message } });
  ...
  useEffect(() => {
    ...
    const hasFunded = fundDetails.funds.some(
      (f: FundDetailsFund) => f.investment?.status === "FUNDED"
    );
    if (hasFunded) {
  ...
  const handleNdaSubmit = async () => {
    ...
    try {
      ...
      logger.error("Error completing gate", { module: "lp-dashboard", metadata: { error: (error as Error).message } });
    } finally {
  ...
  const handleSendNote = async () => {
    ...
    try {
      ...
      logger.error("Error sending note", { module: "lp-dashboard", metadata: { error: (error as Error).message } });
    } finally {
  ...
  const handleSendNoteFromCard = async (content: string): Promise<boolean> => {
    try {
      ...
      logger.error("Error sending note from card", { module: "lp-dashboard", metadata: { error: (error as Error).message } });
      return false;
    }
  };
  ...
  return (
    <>
      ...
      <SubscriptionModal
        ...
        fund={subscriptionStatus?.fund ?? null}
