...
/** DataTransferItem with potential future getAsEntry() method (non-standard) */
      ...
      try {
        ...
        if (existingFoldersResponse.ok) {
          ...
          const existingDataroomFolder = existingFolders.find(
            (folder: { name: string }) => folder.name === dataroomName,
          );
        ...
        const fileResults = await Promise.all(
          Array.from(items, (item) => {
            ...
            const entry =
              ...
              (typeof (item as DataTransferItemWithEntry)?.getAsEntry === "function" &&
                (item as DataTransferItemWithEntry).getAsEntry!()) ??
              null;
            return entry
              ? traverseFolder(
                  entry,
