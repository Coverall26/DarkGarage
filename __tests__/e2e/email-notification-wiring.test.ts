...
const FUND = {
  id: "00000000-0000-4000-a000-000000000001", teamId: "00000000-0000-4000-a000-000000000002", name: "Test Fund I, L.P.",
  ...
  pricingTiers: [], team: { id: "00000000-0000-4000-a000-000000000002", name: "Test Team" },
};
const GP_USER = { id: "00000000-0000-4000-a000-000000000003", email: "gp@test.com", name: "GP Admin" };
const LP_USER = { id: "00000000-0000-4000-a000-000000000004", email: "lp@test.com", name: "LP Investor" };
const INVESTOR = {
  id: "00000000-0000-4000-a000-000000000005", userId: LP_USER.id, fundId: FUND.id, entityType: "INDIVIDUAL",
...
const INVESTMENT = {
  id: "00000000-0000-4000-a000-000000000006", fundId: FUND.id, investorId: INVESTOR.id,
...
describe("Email Notification Wiring", () => {
  ...
  describe("GP Commitment Notification (subscribe API)", () => {
    it("calls sendGpCommitmentNotification after successful subscription", async () => {
      ...
      const txResult = {
        document: { id: "00000000-0000-4000-a000-000000000009", recipients: [{ id: "00000000-0000-4000-a000-00000000000b" }] },
        subscription: { id: "00000000-0000-4000-a000-00000000000a" },
        signingToken: "tok",
        subscriptionAmount: 50000,
        investment: { id: "00000000-0000-4000-a000-000000000006", status: "COMMITTED" },
      };
  ...
  describe("GP Wire Proof Notification (wire-proof API)", () => {
    it("calls sendGpWireProofNotification after LP uploads proof (regular Investment)", async () => {
      ...
      mockPrisma.transaction.create.mockResolvedValueOnce({
        id: "00000000-0000-4000-a000-000000000008", status: "PROOF_UPLOADED",
      });
    ...
    it("does not call GP notification for ManualInvestment proof flow", async () => {
      ...
      (mockPrisma.manualInvestment as any).findUnique.mockResolvedValueOnce({
        id: "00000000-0000-4000-a000-00000000000c", fundId: FUND.id,
      });

      const handler = wrapAppRouteHandler(await import("@/app/api/lp/wire-proof/route"));
      const { req, res } = mkReq("POST", {
        investmentId: "00000000-0000-4000-a000-00000000000c",
        storageKey: "s3://proofs/proof.pdf",
        storageType: "S3_PATH",
        fileType: "application/pdf",
  ...
  describe("Wire Confirmed Notification (GP wire confirm API)", () => {
    it("calls sendWireConfirmedNotification after GP confirms wire", async () => {
      ...
      mockPrisma.user.findUnique.mockResolvedValueOnce({
        ...GP_USER,
        teams: [{ teamId: "00000000-0000-4000-a000-000000000002", role: "ADMIN", status: "ACTIVE" }],
      });

      const txnWithInvestor = {
        id: "00000000-0000-4000-a000-000000000007", fundId: FUND.id, investorId: INVESTOR.id,
      ...
      mockPrisma.fund.findUnique.mockResolvedValueOnce({ id: FUND.id, teamId: "00000000-0000-4000-a000-000000000002" });
      ...
      const { req, res } = mkReq("POST", {
        transactionId: "00000000-0000-4000-a000-000000000007",
