...
export default function LPOnboardClient() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/branding/tenant?teamId=${fundContext.teamId}`)
      ...
      .catch((e) => logger.error("Failed to load branding", { module: "lp-onboard", metadata: { error: (e as Error).message } }));
  ...
  useEffect(() => {
    ...
    fetch(`/api/lp/current-tranche?fundId=${fundContext.fundId}`)
      ...
      .catch((e) => logger.error("Failed to load tranche data", { module: "lp-onboard", metadata: { error: (e as Error).message } }))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.ndaAccepted) {
        ...
        }).catch((e) => logger.error("Failed to sign NDA", { module: "lp-onboard", metadata: { error: (e as Error).message } }));
      }
  ...
  const logCertificationAudit = useCallback(
    (
      ...
      }).catch((e) => logger.error("Failed to capture accreditation audit", { module: "lp-onboard", metadata: { error: (e as Error).message } }));
    },
    [fundContext?.fundId],
  );
        ...
        <Card className="bg-gray-800/50 border-gray-700 backdrop-blur">
          <CardHeader>
            <CardTitle className="text-white">
              ...
              {step === 6 && (fundContext?.kycRequired && !kycApproved ? "Identity Verification" : "Investment Commitment")}
            ...
            <CardDescription className="text-gray-400">
              ...
              {step === 6 && (fundContext?.kycRequired && !kycApproved ? "Complete KYC/AML verification before investing" : "Select the number of units you'd like to commit")}
          ...
          <CardContent>
            ...
            {step === 9 ? (
              ...
              /* Step 6: KYC Gate (if required) → Investment Commitment */
              fundContext?.kycRequired && !kycApproved ? (
                  ...
                  <InvestorTypeStep
                    ...
                    onAutoSave={(data) => {
                      saveProgress(2, data as unknown as Record<string, unknown>, {
                        personal_info: true,
                        entity_type: false,
                      }).catch((e) => logger.error("Failed to save progress", { module: "lp-onboard", metadata: { error: (e as Error).message } }));
                    }}
