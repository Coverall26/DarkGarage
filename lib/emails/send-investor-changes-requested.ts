...
export async function sendInvestorChangesRequestedEmail({
  ...
  try {
    ...
    if (!investor?.user?.email) {
      logger.warn("No email found for changes-requested notification", { module: "send-investor-changes-requested", investorId });
      return;
    }
    ...
    logger.error("Failed to send changes-requested notification", { module: "send-investor-changes-requested", error: String(error), investorId, fundId });
  }
}
