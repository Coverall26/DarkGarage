...
export default function DeckGeneratorUpload() {
  ...
  useEffect(() => {
    ...
    if (reportUrl) {
      const blob = fetch(reportUrl)
        ...
        .catch((error) => logger.error("Error fetching report", { module: "special-upload", metadata: { error: (error as Error).message } }));
  ...
  const handleBrowserUpload = async (event: any) => {
    ...
    try {
      ...
      logger.error("An error occurred while uploading the file", { module: "special-upload", metadata: { error: (error as Error).message } });
      setCurrentFile(null);
      setUploading(false);
    }
  };
