...
export async function createFolderInBoth({
  ...
  analytics: { capture?: (event: string, properties?: Record<string, unknown>) => void } | null;
  ...
  try {
    ...
    logger.error("An error occurred while creating the folder in both locations", { module: "folders", metadata: { error: (error as Error).message } });
    setRejectedFiles([
      {
        fileName: name,
        message:
