...
- **Affordable + scalable via third-party APIs:** Use best-in-class vendors via clean adapters. E-signature is native via SignSuite.
...
- E-Signature: SignSuite (native, self-hosted, zero external API cost)
...
Active by default: SignSuite, Secure Storage, Audit Logging, Email Notifications, Manual Wire. Optional: KYC/AML, Stripe ACH, QuickBooks, K-1 automation.
...
**Step 2: NDA E-Signature** — PDF viewer + SignSuite. Stored as Document type: NDA.
...
- Part B: Auto-filled document review & sign via SignSuite
...
| SignSuite (native e-signature) | **DONE** | `app/sign/`, `components/sign/` |
