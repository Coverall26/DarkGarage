 ...
 *
 * Four-tier CRM subscription model:
 *   FREE:     $0/mo — 20 contacts, 10 e-sigs/mo, 40 signers, table view, 2 templates
 *   CRM_PRO:  $29/mo — Unlimited contacts, 25 e-sigs/mo, 100 signers, Kanban, 5 templates
 *   BUSINESS: $39/mo — Unlimited contacts/signers, 75 e-sigs/mo, 10 templates, analytics
 *   FUNDROOM: $79/mo — Unlimited everything, compliance pipeline, investor onboarding
 *
...
export type CrmPlanSlug = "FREE" | "CRM_PRO" | "BUSINESS" | "FUNDROOM";
...
const CRM_PRICE_IDS = {
  ...
  BUSINESS: {
    monthly: isProduction
      ? (process.env.STRIPE_BUSINESS_MONTHLY_PRICE_ID ?? "price_business_monthly_prod")
      : (process.env.STRIPE_BUSINESS_MONTHLY_PRICE_ID ?? "price_business_monthly_test"),
    yearly: isProduction
      ? (process.env.STRIPE_BUSINESS_YEARLY_PRICE_ID ?? "price_business_yearly_prod")
...
export const CRM_PLANS: Record<CrmPlanSlug, CrmPlan> = {
  ...
  CRM_PRO: {
    ...
    yearlyPriceDollars: 23,
    price: {
      monthly: { amount: 2900, priceId: CRM_PRICE_IDS.CRM_PRO.monthly },
      yearly: { amount: 2300, priceId: CRM_PRICE_IDS.CRM_PRO.yearly },
    },
    features: [
      "Unlimited contacts",
  ...
  BUSINESS: {
    ...
    yearlyPriceDollars: 32,
    price: {
      monthly: { amount: 3900, priceId: CRM_PRICE_IDS.BUSINESS.monthly },
      yearly: { amount: 3200, priceId: CRM_PRICE_IDS.BUSINESS.yearly },
    },
    features: [
...
// Build reverse lookup from CRM_PRO, BUSINESS, and FUNDROOM plans
for (const plan of [CRM_PLANS.CRM_PRO, CRM_PLANS.BUSINESS, CRM_PLANS.FUNDROOM]) {
