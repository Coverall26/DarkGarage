  ...
  if (cachedDurations) {
    ...
    if (allViewIds.length > 0) {
      try {
        ...
        logger.error("Error fetching durations from PageView", { module: "teams", metadata: { error: (error as Error).message } });
        // Fall back to zeros
        for (const view of groupedViews) {
    ...
    try {
      ...
      if (withDuration === "true") {
        try {
          ...
          logger.error("Error processing duration batches", { module: "teams", metadata: { error: (error as Error).message } });
      ...
      logger.error("Viewer Details API Error", { module: "teams", metadata: { error: (error as Error).message } });
    ...
    try {
      ...
      if (redis) {
        try {
          ...
          logger.error("Error clearing cache", { module: "teams", metadata: { error: (cacheError as Error).message } });
        }
      }
      ...
      logger.error("Error revoking viewer access", { module: "teams", metadata: { error: (error as Error).message } });
      errorhandler(error, res);
    }
  } else {
