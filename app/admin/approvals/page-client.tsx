...
export default function ApprovalsPageClient() {
  ...
  useEffect(() => {
    fetch("/api/admin/fund/list")
      .then((res) => res.json())
      .then((data) => {
        if (data.funds?.length > 0) {
          setFunds(data.funds);
          setTeamId(data.funds[0]?.teamId || null);
        }
        setLoaded(true);
      })
      .catch((e) => {
        logger.error("Failed to load approvals context", { module: "admin-approvals", metadata: { error: (e as Error).message } });
        setLoaded(true);
      });
  }, []);
