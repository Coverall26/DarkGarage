...
const mockRequireLPAuthAppRouter = jest.fn().mockResolvedValue({
  userId: "user-1",
  email: "lp@test.com",
...
jest.mock("@/lib/investor/approval-pipeline", () => ({
  determineCurrentStage: jest.fn().mockImplementation((inv: Record<string, unknown>) => {
...
describe("LP Upload Flow E2E", () => {
  ...
  describe("Flow A: Wire Proof Upload", () => {
    ...
    it("should reject unauthenticated requests", async () => {
      mockRequireLPAuthAppRouter.mockResolvedValueOnce(
  ...
  describe("Flow B: LP Document Upload", () => {
    ...
    it("should accept document upload with base64 file data", async () => {
      // user.findUnique is called by getLPDocumentAuthContextAppRouter for permissions
      (prisma.user.findUnique as jest.Mock).mockResolvedValueOnce({
        id: "user-1",
        email: "lp@test.com",
        role: "USER",
        investorProfile: { id: "investor-1", fundId: "fund-1" },
        teams: [],
      });
  ...
  describe("Pending Details API", () => {
    ...
    beforeEach(async () => {
      pendingDetailsHandler = wrapAppRouteHandler(
        await import("@/app/api/admin/fund/[fundId]/pending-details/route"),
        { fundId: "fund-1" },
      );
    });
