...
// App Router equivalent: app/api/transactions/pending-confirmation/route.ts
...
export default async function handler(
  ...
  try {
    ...
    // Find user with admin team memberships + their funds in one query
    ...
    // Extract fund IDs from the nested team query (no separate fund query needed)
    const gpFunds = user.teams.flatMap((m) => m.team.funds);
    const gpFundIds = gpFunds.map((f) => f.id);
    const fundMap = new Map(gpFunds.map((f) => [f.id, f]));
    ...
    logger.error("Error fetching pending confirmations", { module: "transactions", metadata: { error: (error as Error).message } });
