...
// Production guard: refuse to seed production database without explicit opt-in
...
async function main() {
  ...
  // 5b. Create Brand record for team
  if (team && !dryRun) {
    ...
    if (!existingBrand) {
      await prisma.brand.create({
        data: {
          teamId: team.id,
          logo: null, // Cloud storage — set after logo upload
          brandColor: BFG_CONFIG.org.brandColor,
          accentColor: BFG_CONFIG.org.accentColor,
          welcomeMessage:
            "Welcome to the Bermuda Franchise Fund I investor portal. All documents are confidential.",
        },
      });
      console.log("  Created Brand for team");
    } else {
      console.log("  Brand exists for team");
    }
  }
  ...
  // 10b. Create Offering Page for the fund
  if (fund && team && !dryRun) {
    const offeringSlug = "bermuda-club-fund";
    const existingOffering = await prisma.offeringPage.findUnique({
      where: { slug: offeringSlug },
    });

    if (!existingOffering) {
      await prisma.offeringPage.create({
        data: {
          fundId: fund.id,
          teamId: team.id,
          slug: offeringSlug,
          isPublic: true,
          heroHeadline: "Bermuda Franchise Fund I, L.P.",
          heroSubheadline:
            "A 506(c) private placement fund acquiring and operating premium franchise units across the Southeast United States.",
          heroBadgeText: "Now Open — Early Investor Tranche",
          offeringDescription:
            "Bermuda Franchise Group is raising $9.55M to acquire, develop, and operate franchise units in high-growth markets. The fund targets multi-unit franchise operations with proven unit economics and strong brand recognition.",
          keyMetrics: [
            { label: "Target Raise", value: "$9.55M" },
            { label: "Minimum Investment", value: "$90,000" },
            { label: "Management Fee", value: "2.5%" },
            { label: "Carried Interest", value: "20%" },
            { label: "Hurdle Rate", value: "8%" },
            { label: "Fund Term", value: "8 + 2 years" },
          ],
          highlights: [
            {
              title: "Proven Unit Economics",
              description:
                "Each franchise unit targeted has demonstrated positive cash flow with AUV exceeding $1.2M.",
            },
            {
              title: "Experienced Operators",
              description:
                "Management team brings 20+ years of combined franchise operations experience.",
            },
            {
              title: "Southeast Growth Markets",
              description:
                "Targeting high-growth MSAs in Florida, Georgia, and the Carolinas.",
            },
          ],
          dealTerms: [
            { label: "Fund Structure", value: "Wyoming Limited Partnership" },
            { label: "SEC Exemption", value: "Rule 506(c)" },
            { label: "Waterfall", value: "European (Whole Fund)" },
            { label: "Preferred Return", value: "8% hurdle" },
            { label: "GP Commitment", value: "2% of total raise" },
          ],
          timeline: [
            {
              date: "2026-01-15",
              title: "Fund Formation",
              description: "LP Agreement finalized, fund established",
              status: "completed",
            },
            {
              date: "2026-02-01",
              title: "Early Investor Tranche Open",
              description: "Tranche 2 at $90,000/unit — 25 units available",
              status: "current",
            },
            {
              date: "2026-06-01",
              title: "First Acquisition Target",
              description: "LOI on first multi-unit franchise package",
              status: "upcoming",
            },
          ],
          leadership: [
            {
              ...
              bio: "20+ years in franchise development and multi-unit operations across the Southeast.",
            },
            {
              name: "Ricardo Ciesco",
              title: "Fund Administrator",
              bio: "Platform operations and investor relations. Technology and compliance oversight.",
            },
          ],
          ctaText: "I Want to Invest",
          ctaSecondary: "Request More Info",
          emailGateEnabled: true,
          disclaimerText:
            "This offering is made only to accredited investors pursuant to Rule 506(c) of Regulation D under the Securities Act of 1933. Past performance is not indicative of future results.",
        },
      });
      console.log(`  Created OfferingPage: /offering/${offeringSlug}`);
    } else {
      console.log(`  OfferingPage exists: /offering/${offeringSlug}`);
    }
  }
  ...
  console.log("");
  console.log("  Offering Page:  /offering/bermuda-club-fund");
  console.log("");
...
async function cleanBFGData() {
  ...
  for (const teamId of teamIds) {
    ...
    await prisma.offeringPage.deleteMany({ where: { teamId } });
    ...
    await prisma.brand.deleteMany({ where: { teamId } });
