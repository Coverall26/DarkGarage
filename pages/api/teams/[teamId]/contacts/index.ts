...
async function handleSearch(
  ...
  try {
    ...
    logger.error("Error searching contacts", { module: "teams", metadata: { error: (error as Error).message } });
...
async function handleCreate(
  ...
  try {
    ...
    reportError(error as Error);
    logger.error("Error creating contact", { module: "teams", metadata: { error: (error as Error).message } });
    return res.status(500).json({ error: "Internal server error" });
  }
}
