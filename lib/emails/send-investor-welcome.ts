...
export async function sendInvestorWelcomeEmail(
  ...
  try {
    ...
    if (!user?.email) {
      logger.warn("No user/email found for welcome email", { module: "send-investor-welcome", userId });
      return;
    }
    ...
    logger.error("Failed to send investor welcome email", { module: "send-investor-welcome", error: String(error), userId });
  }
}
...
export async function sendInvestorWelcomeEmailWithFund(
  ...
  try {
    ...
    if (!user?.email) {
      logger.warn("No user/email for welcome+fund email", { module: "send-investor-welcome", userId, fundId });
      return;
    }
    ...
    logger.error("Failed to send investor welcome+fund email", { module: "send-investor-welcome", error: String(error), userId, fundId });
  }
}
