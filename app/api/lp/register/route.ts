 ...
 *
 * INTENTIONALLY_PUBLIC: Registration endpoint — no session auth required because
 * LPs register before they have an account. Protected by rate limiting (5 req/60s),
 * bot protection, Zod validation, and paywall checks.
 *
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (user) {
      if (!user.investorProfile) {
        ...
        await prisma.investor.create({
          data: {
            userId: user.id,
            ...investorData,
          } as Prisma.InvestorUncheckedCreateInput,
        });
      } else {
      ...
      user = await prisma.user.create({
        data: {
          ...
          investorProfile: {
            create: investorData as Prisma.InvestorCreateWithoutUserInput,
          },
        },
        include: { investorProfile: true },
      });
    ...
    return errorResponse(error, "lp-register");
  }
}
